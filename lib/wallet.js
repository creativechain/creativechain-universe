...
function validateSendCoins() {
    ...
    trantor.rpcWallet.validateAddress(address, function (err, result) {
        if (err) {
            ...
            if (result.isvalid) {
                sendCoins(address, amount, null, subtractFee);
...
function sendCoins(address, amount, feeKb = 0, subtractFeeFromAmount = 0) {
    ...
    let outputs = {};
    outputs[address] = Coin.parseCash(amount, 'CREA').getScaleValue();

    trantor.createTransaction(outputs, feeKb, subtractFeeFromAmount, function (err, builtTx) {
        if (err) {
            handleRpcError(err);
        } else {
            trantor.emit('core.transaction.build', builtTx);
        }
    });
