...
function loadAddressBook() {
    trantor.dbrunner.send('getAddressBook', function (err, result) {
        if (err) {
            console.error(err);
...
function putTransactions() {
    trantor.rpcWallet.listTransactions('*', 9999, function (err, result) {
...
function putRequestPayments() {
    ...
    paymentList.html('');
    setTimeout(function () {
        trantor.dbrunner.send('getAllPaymentRequest', function (err, res) {
            if (err) {
                console.error(err)
            } else {
                //console.log(res);
...
function putBalance() {
    ...
    trantor.rpcWallet.getBalance(function (err, result) {
...
function validateSendCoins() {
    ...
    trantor.rpcWallet.validateAddress(address, function (err, result) {
        if (err) {
            handleRpcError(err);
        } else {
            console.log(result);
...
function updateContact(address, label) {
    setTimeout(function () {
        trantor.dbrunner.send('updateAddressBook', address, label, function (err, result) {
...
function sendCoins(address, amount, feeKb = 0, subtractFeeFromAmount = 0) {
    ...
    trantor.getSpendables(0, function (err, spendables) {
        if (err) {
            ...
            trantor.rpcWallet.getRawChangeAddress(function(err, result) {
                if (err) {
                    console.error(err);
                } else {
                    ...
                    if (txBuilder.complete) {
                        ...
                        trantor.emit('onBeforeTransactionSend', txBuilder, creaBuilder);
                    } else {
                        modal.error({
...
function requestPayment() {
    ...
    trantor.rpcWallet.getNewAddress(function (err, result) {
        ...
        trantor.dbrunner.send('insertPaymentRequest', address, amount, date, label, message, function (err, res) {
            putRequestPayments();
            clearPaymentFields();
        })
    });
...
function showPayment(address) {
    trantor.dbrunner.send('getPaymentRequest', address, function (err, payment) {
        payment = payment[0];
    ...
    dialog.showSaveDialog(null, {
        ...
        if (fileName) {
            ...
            setTimeout(function () {
                ...
                trantor.emit('onNotification', lang.Files, notifBody, './assets/img/notification/wallet.png', 10);
            }, timeout)
        }
    })
}
...
function loadWalletFile() {
    dialog.showOpenDialog((fileNames) => {
        if(fileNames){
            ...
            let destFile = CONSTANTS.WALLET_DIR + 'wallet.dat';
...
function deleteAddressBookEntry(address) {
    trantor.dbrunner.send('removeAddress', address);
    setTimeout(function () {
        loadAddressBook()
    }, 200);
}