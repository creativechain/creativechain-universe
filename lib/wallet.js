...
function loadAddressBook() {
    trantor.dbrunner.getAddressBook(function (err, result) {
        if (err) {
            console.error(err);
...
function putRequestPayments() {
    ...
    paymentList.html('');
    trantor.dbrunner.getAllPaymentRequest(function (err, res) {
        if (err) {
            console.error(err)
        } else {
            //console.log(res);
...
function validateSendCoins() {
    ...
    trantor.rpcWallet.validateAddress(address, function (err, result) {
        if (err) {
            ...
            if (result.isvalid) {
                sendCoins(address, amount, 405000, subtractFee);
...
function updateContact(address, label) {
    trantor.dbrunner.updateAddressBook(address, label, function (err, result) {
...
function sendCoins(address, amount, feeKb = 0, subtractFeeFromAmount = 0) {
    ...
    trantor.getSpendables(0, function (err, spendables) {
        if (err) {
            ...
            let txBuilder = new TransactionBuilder(trantor.configuration.network, feeKb);
            trantor.rpcWallet.getRawChangeAddress(function(err, result) {
                if (err) {
                    ...
                    if (txBuilder.complete) {
                        ...
                        trantor.emit('core.transaction.build', txBuilder, creaBuilder);
                    } else {
                        modal.error({
...
function requestPayment() {
    ...
    trantor.rpcWallet.getNewAddress(function (err, result) {
        ...
        trantor.dbrunner.insertPaymentRequest(address, amount, date, label, message, function (err, res) {
            putRequestPayments();
            clearPaymentFields();
        })
    });
...
function showPayment(address) {
    trantor.dbrunner.getPaymentRequest(address, function (err, payment) {
        payment = payment[0];
...
function deleteAddressBookEntry(address) {
    trantor.dbrunner.removeAddress(address);
    loadAddressBook()
}