...
function loadTopNotifications() {
    getUserAddress(function (userAddress) {
        trantor.dbrunner.getUnviewedNotifications(99999, function (err, nots) {
            if (err) {
                console.error(err);
            } else {
                ...
                let processNotif = function (notif) {
                    ...
                    switch (notif.type) {
                        case TrantorConstants.TYPE.CONTENT:
                            trantor.dbrunner.getMediaByAddress(notif.resource, userAddress, function (err, result) {
                                if (err) {
                                    console.error(err);
                                } else if (result.length > 0) {
                                    result = result[0];
                                    notif.data = result;
                                    notif.isUpdate = result.author === CONSTANTS.UPDATING_USER;
                        ...
                        case TrantorConstants.TYPE.FOLLOW:
                            trantor.dbrunner.getAuthor(notif.author, userAddress, function (err, result) {
                                if (err) {
                                    console.error(err);
                                } else if (result.length > 0) {
                                    result = result[0];
...
function loadPageNotifications() {
    getUserAddress(function (userAddress) {
        trantor.dbrunner.getNotifications(50, function (err, nots) {
            if (err) {
                console.error(err);
            } else {
                let notifications = [];
                ...
                let processNotif = function (notif) {
                    ...
                    trantor.dbrunner.getAuthor(notif.author, userAddress, function (err, follows) {
                        if (err) {
                            console.log(err);
                        } else {
                            ...
                            switch (notif.type) {
                                ...
                                case TrantorConstants.TYPE.LIKE:
                                    trantor.dbrunner.getAuthor(notif.author, userAddress, function (err, result) {
                                        ...
                                        trantor.dbrunner.getMediaByAddress(notif.resource, userAddress, function (err, media) {
                                            notif.data = result;
                                            notif.icon = './assets/img/like0.png';
                                            notif.description = lang.NotifLike;
                                            notif.mediaIcon = getDefaultImageAndColor(media[0].content_type, media[0].featured_image);
                                            notifications.push(notif);
                                            nextNotif();
                                        });
                                ...
                                case TrantorConstants.TYPE.COMMENT:
                                    trantor.dbrunner.getAuthor(notif.author, userAddress, function (err, result) {
                                        ...
                                        trantor.dbrunner.getMediaByAddress(notif.resource, userAddress, function (err, media) {
                                            console.error('NOTIF C', notif, media[0]);
                                            notif.data = result;
                                            notif.icon = './assets/img/comments.png';
                                            notif.description = lang.NotifComment;
                                            notif.mediaIcon = getDefaultImageAndColor(media[0].content_type, media[0].featured_image);
