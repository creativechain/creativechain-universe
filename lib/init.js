...
console.log(appPath);
let torrentClient = new Torrents(trantor);

trantor.localStorage = localStorage;
let settings = FileStorage.load();

const SESSION_DURATION_MILLIS = 3000;//settings.getKey('session-time', 1800) * 1000;
...
$(function () {
  ...
  $('body').on('click', '.modal-session-expired', function(evt) {
    // si clican el fondo negro que no se cierre el modal
    evt.stopPropagation();
    evt.preventDefault();
  })

  $('body').on('click', '#modal-session-expired_cancel', closeApp);

  $('body').on('click', '#modal-session-expired_Ok', function(){
    let valorModal = $('#modal-session-expired_input').val();
    console.log('Input Modal: ', valorModal);
  });
})
