...
window.onbeforeunload = function (event) {
    ...
    if (!onSplash) {
        if (!mustCloseApp) {
            console.log('Creating close timeout');
            setTimeout(function () {
        ...
        event.returnValue = mustCloseApp;
...
let TX_CONTENT_AMOUNT = Coin.parseCash(0.005, 'CREA');
...
class FileClient {
    ...
    start() {
        let that = this;
        let lockFile = OS.getUserDir() + '.jsipfs/repo.lock';
    ...
    createFile(file, destPath, callback) {
        ...
        let files = [data];
        this.node.files.add(fileBuffer, function (err, resultFiles) {
            if (err) {
                console.error(err)
            } else if (callback) {
                if (resultFiles.length > 0) {
                    ...
                    setTimeout(function () {
                        ...
                        let options = {
                            url: credentials.endpoints.IPFS + ipfsData.hash,
                            method: 'GET',
                            headers: headers,
                        ...
                        request(options, function (error, response, body) {
                            console.log('IPFS Shared', options.url, response, body);

                            options.url = credentials.endpoints.IPFS2;
                            options.method = 'POST';
                            request(options, function (error, response, body) {
                                console.log('IPFS Shared', options.url, response)
                            });
                        });
                    ...
                    console.error('IPFS not build files', fileBuffer, resultFiles)
                }
    ...
    downloadFile(contentAddress, magnet, callback, privateContent = false) {
        let that = this;
        if (magnet) {
            let path = Constants.DATA_FOLDER + contentAddress;
            if (privateContent) {
                path += '-p'
            }
