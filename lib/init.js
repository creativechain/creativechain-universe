...
class Torrents {
    constructor() {
        ...
        //this.worker = cp.fork(__dirname + '/lib/torrent.js');
        this.callbacks = {};
        this.torrentIds = {};
    }
...
/*        this.worker.on('message', function (event) {
        ...
        ipcRenderer.on('main-message', function (event, arg) {
            ...
            that.processData(arg);
        })
        this.send({id: 0, code: TorrentMsgCode.LOG, data: 'start'})
    }

    send(data) {
        //this.worker.send(data);
        ipcRenderer.send('platform-message', data)
    }
...
/*        let that = this;
        ...
        this.containsTorrent(destFile, function (contains, torrent) {
            if (contains) {
                if (callback) {
                    callback(torrent, destFile)
                }
            } else {
...
/*        let that = this;
        ...
        this.containsTorrent(file, function (contains) {
            if (contains) {
        ...
        if (magnet) {
            ...
            that.containsTorrent(magnet, function (contains) {
                if (!contains) {
                    ...
                    that.client.add(magnet, {path: path}, function (torrent) {
                        let file = path + torrent.name;
                        console.log('Torrent added', torrent.infoHash, file);
                        that.torrentIds[file] = torrent.infoHash;
                    ...
                    that.getTorrent(magnet, function (torrent) {
                        ...
                        let file = path + parsedTorrent.name;
...
/*        if (File.exist(torrent)) {
        ...
        this.client.remove(torrent, callback);*/
        let data  = {
            id: Utils.randomNumber(100000000, 999999999),
