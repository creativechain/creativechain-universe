        ...
        if (password.length > 0 && repeatPassword.length > 0 && password === repeatPassword) {
            ...
            trantor.encryptWallet(password, function (err, result) {
                if (err) {
                    ...
                    trantor.emit('onNotification', lang.Wallet, lang.WalletEncrypted, './assets/img/notification/wallet.png', 3);
                    trantor.stop(false);
...
function createBackup() {
    ...
    let walletPath = CONSTANTS.BIN_DIR + (isTestnet ? 'testnet3' + '/' : '') + 'wallet.dat';
