...
function loadProfileView(profileAddress, isUser = false) {
    page = PAGES.PROFILE_POSTS;

    loadProfileData(profileAddress, isUser);
    showProfileHome();
}
...
function loadProfileData(profileAddress, isUser = false) {
    ...
    if (isUser) {
        ...
        $('#ui-profile-filter-blockeds').removeClass('hidden');
    } else {
        getUserAddress(function (userAddress) {
            onUserAddress(userAddress);
        });
        $('#ui-profile-filter-notifications').addClass('hidden');
        $('#ui-profile-filter-blockeds').addClass('hidden');
    }
}
        ...
        if (err) {
            ...
            if (!profileVueFollowers) {
                profileVueFollowers = new Vue({
                    ...
                    methods: {
                        resolveAvatar: resolveAvatar,
                        loadProfileData: loadProfileView,
                        followButtonEnter: followButtonEnter,
                        followButtonLeave: followButtonLeave,
                        performFollow: performFollow,
                    }
...
function removeProfileContent(contentAddress) {
    let length = articleProfileVueList.$data.userPosts.length;
    for (let x = 0; x < length; x++) {
        let post = articleProfileVueList.$data.userPosts[x];
        if (post.address === contentAddress) {
            articleProfileVueList.$data.userPosts.splice(x, 1);
            break;
        }
    }
}
