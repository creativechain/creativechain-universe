...
const TX_CONTENT_AMOUNT = Coin.parseCash(0.001, 'CREA').value;
const TX_FEE_KB = Coin.parseCash(0.00405, 'CREA').value;
...
class MediaData extends ContentData {
     ...
     * @param {Array} hashes
     */
    constructor(title, description, contentType, license, userAddress, contentAddress, tags, price, publicContent, privateContent, hashes) {
...
class TransactionBuilder {
    ...
    addOutput(address, amount) {
        ...
        if (this.isAddressInOutputs(address)) {
            ...
            this.outputSumAmount += txOut.value;
        }
    }
    ...
    completeTx(spendables) {
        ...
        for (let x = 0; x < spendables.length; x++) {
            ...
            if (spendable.matchAmount(matchAmount)) {
                this.addInput(spendable.txId, spendable.vout, spendable.address, spendable.value);
                this.complete = true;
            }
        }
        ...
        for (let x = 0; x < spendables.length; x++) {
            ...
            if (spendable.value < amountNet) {
                utxoSumAmount += spendable.value;
                this.addInput(spendable.txId, spendable.vout, spendable.address, spendable.value);
        ...
        for (let x = 0; x < spendables.length; x++) {
            ...
            if (spendable.value === amountNet) {
                this.addInput(spendable.txId, spendable.vout, spendable.address, spendable.value);
                this.complete = true;
            } else if (spendable.value > amountChange) {
                this.addInput(spendable.txId, spendable.vout, spendable.address, spendable.value);
                this.addOutput(this.changeAddress, this.inputSumAmount - amountChange);
                this.complete = true;
            }
...
class Spendable {
    ...
    static spendablesFrom(amount, spendables, extraSize = 0) {
        ...
        for (let x = 0; x < spendables.length; x++) {
            let spendable = spendables[x];
            if (spendable.value > matchAmount) {
    ...
    static parseJson(json) {
        ...
        json.forEach(function (spend) {
            ...
            spend.value = parseFloat(spend.value) + 0.000000001;
            spendables.push(new Spendable(spend.txid, spend.vout, spend.address, spend.value, spend.confirmations, spend.spendable, spend.scriptPubKey))
        });
...
class Storage {
    ...
    getAuthor(address, callback) {
        this.query("SELECT a.*, " +
            "(SELECT count(*) FROM 'Like' l WHERE l.author = '" + address + "') AS likes, " +
            "(SELECT count(*) FROM 'Comment' c WHERE c.author = '" + address + "') AS comments, " +
            "(SELECT count(*) FROM 'Media' m WHERE m.author = '" + address + "') AS publications, " +
            "(SELECT count(*) FROM 'Following' f WHERE f.type = 5 AND f.followed_address = '" + address +"') AS followers, " +
    ...
    insertNotification(author, type, resource, date, callback) {
        ...
        inserNotification.run(author, type, resource, date, function (err) {
            if (callback) {
                callback(err);
            }
        });
     ...
     * @param callback
     */
    getFollowers(userAddress, callback) {
        this.query("SELECT f.follower_address, " +
            ...
            "(u.address = f.follower_address) WHERE f.followed_address = '" + userAddress + "' AND f.type = " + PUBLICATION.TYPE.FOLLOW, callback);
    }
     ...
     * @param callback
     */
    getFollowing(userAddress, callback) {
        this.query("SELECT f.followed_address, " +
            "u.* FROM 'Following' f " +
            "LEFT JOIN (SELECT a.*, " +
            "(SELECT count(*) FROM 'Following' f2 WHERE a.address = f2.followed_address AND f2.follower_address = '" + userAddress + "' AND f2.type = " + PUBLICATION.TYPE.FOLLOW + ") AS is_following," +
            "(SELECT t.file FROM 'Torrent' t WHERE t.magnet = a.avatar) AS avatarFile FROM 'Author' a) u ON " +
            "(u.address = f.followed_address) WHERE f.follower_address = '" + userAddress + "' AND f.type = " + PUBLICATION.TYPE.FOLLOW, callback);
    }
    ...
    getFollowingData(followerAddress, followedAddress, type, callback) {
        this.query("SELECT * FROM 'Following' WHERE follower_address = '" + followerAddress + "' AND followed_address = '" + followedAddress + "'" +
            "AND type = " + type + ";", callback)
    }
    ...
    getMediaByFollowerAddress(followerAddress, callback) {
        this.query("SELECT f.follower_address, n.* FROM Following f\n" +
            ...
            ") n \n" +
            "ON (n.author = f.followed_address) WHERE f.follower_address = '" + followerAddress + "' ORDER BY n.creation_date DESC;", callback)
    }
                        ...
                        if (err) {
                            ...
                            if (txBuilder.complete) {
                                txBuilder.inputs.forEach(function (input) {
                                    creaBuilder.addInput(input.txId, input.vout);
                                });

                                creaBuilder.addOutput(opReturnData, 0);

                                txBuilder.outputs.forEach(function (output) {
                                    creaBuilder.addOutput(output.address, output.value);
                                });
...
if (module) {
    module.exports = {CoinUri, Currency, UnknownCurrency, FiatCurrency, CryptoCurrency, Eur, Usd, Mxn, Pln, Btc, Crea, Coin,
        ...
        Follow, Unfollow, TxInput, TxOutput, DecodedTransaction, Network, NodeConfiguration, EventHandler, Trantor, Notifications, TorrentMsgCode}
}