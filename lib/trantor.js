...
class Storage {
    ...
    getNotifications(callback, limit = 50) {
        this.query('SELECT * FROM Notifications ORDER BY on_data DESC LIMIT ' + limit + ';', callback);
    }
    ...
    getUnviewedNotifications(callback, limit = 50) {
        this.query('SELECT * FROM Notifications WHERE viewed = 0 ORDER BY on_data DESC LIMIT ' + limit + ';', callback);
    }

    setViewedNotifications(callback) {
        this.query('UPDATE Notifications SET viewed = 1 WHERE viewed = 0;')
    }
    ...
    getMediaByFollowerAddress(followerAddress, callback) {
        this.query("SELECT f.follower_address, n.* FROM Following f\n" +
            ...
            ") n \n" +
            "ON (n.author = f.followed_address) WHERE f.follower_address = '" + followerAddress + "' ORDER BY n.creation_date DESC;")
    }
    ...
    getMediaByFollowedAddress(followedAddress, callback) {
        this.query("SELECT f.followed_address, n.* FROM Following f\n" +
            ...
            "(SELECT count(*) FROM 'Media' m2 WHERE m2.author = a.address) AS publications FROM Author a) u ON (u.user_address = m.author)\n" +
            ") n \n" +
            "ON (n.author = f.follower_address) WHERE f.followed_address = '" + followedAddress + "' ORDER BY n.creation_date DESC;")
    }
...
class Trantor {
    ...
    checkBinariesExists(callback) {
        ...
        let checksum;
        File.download(Constants.CHECKSUMS_URL, checksumFile, null, function (error) {
            if (error) {
                checksum = true;
...
class Torrents {
    ...
    constructor() {
        ...
        this.torrentClient.setMaxListeners(20);
    }
    ...
    containsTorrent(torrentId, callback) {
        this.getTorrent(torrentId, function (torrent) {
            if (callback) {
    ...
    getTorrent(torrentId, callback) {
        ...
        let onGetTorrent = function () {
            if (callback) {
                callback(that.torrentClient.get(id));
            }
        };

        if (File.exist(torrentId)) {
            createTorrent(torrentId, function (err, torrent) {
                if (!err) {
                    id = torrent.infoHash;
                }
    ...
    addFile(parentFolder, file, magnetUri, callback) {
        ...
        let onDownloadTorrent = function () {
            ...
            that.downloadTorrent(contentAddress, magnetUri, callback, privateContent);
        };
    ...
    seedFile(file, callback) {
        ...
        setTimeout(function () {
            ...
            that.containsTorrent(file, function (contains) {
                if (contains) {
                    that.getTorrent(file, function (torrent) {
                        onTorrent(torrent);
                    });
                } else {
    ...
    downloadTorrent(contentAddress, magnet, callback, privateContent = false) {
        ...
        if (magnet) {
            ...
            setTimeout(function () {
                ...
                that.containsTorrent(magnet, function (contains) {
                    if (!contains) {
                        ...
                        that.getTorrent(magnet, function (torrent) {
