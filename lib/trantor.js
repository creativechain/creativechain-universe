...
class OS {
    ...
    static run(command, args, callback) {
        ...
        commandProcess.on('close', function (code, signal) {
            console.log('Process closed: Code ', code, signal);
        });

        commandProcess.on('exit', function (code, signal) {
            console.log('Process exited: Code ', code, signal);
        });
...
class Trantor {
    ...
    initClients(callback) {
        ...
        OS.run(daemon, ['-usehd', '-datadir=' + folder], function (result) {
            console.log('Starting daemon', daemon, result.toString('utf8'));
            callCallback();
        });
