...
class ContentData {
    ...
    setCompression() {
        ...
        this.compressed = length >= 160 ? 1 : 0;
    }
    /**
     *
     * @returns {Buffer}
    ...
    static deserializeData(data) {
        ...
        if (contentData) {
            contentData.compressed = compressed;
...
class TxOutput {
    ...
    hasData() {
        ...
        let dataLength = varint.decode(scriptBuffer.slice(1), 0);
        let sliceLength = varint.decode.bytes + 1;
        let dataHex = scriptBuffer.slice(sliceLength).toString('hex');
    ...
    getRawData() {
        if (this.hasData()) {
            let scriptBuffer = this.getBufferedScript();
            let dataLength = varint.decode(scriptBuffer.slice(1), 0);
            let sliceLength = varint.decode.bytes + 2;
            let dataHex = scriptBuffer.slice(sliceLength).toString('hex');
            console.log(dataHex);
...
/*                    setTimeout(function () {
    ...
    buildDataOutput(data, callback) {
        if (data.compressed) {
            Utils.compress(data, COMPRESSION_LEVEL, function (compressed, error) {
            ...
            let compressed = ContentData.serializeNumber(PUBLICATION.MAGIC_BYTE) + data.serialize().toString('hex');
            console.log('Final data:', compressed.length, compressed.toString('hex'));
            let ret = creativecoin.script.compile([
                creativecoin.opcodes.OP_RETURN,
                compressed
