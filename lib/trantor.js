...
class ContentData {
    ...
    static serializeNumber(number) {
        ...
        let neededChars = pairChars ? numberHex.length : numberHex.length + 1;
...
class Author extends ContentData {
    ...
    deserialize(buffer, offset) {
        ...
        this.address = creativecoin.address.toBase58Check(this.address, Network.TESTNET.pubKeyHash);
        offset += 20;
...
class MediaData extends ContentData {
    ...
    serialize() {
        ...
        let tags = JSON.stringify(this.tags);
        bufferHex += ContentData.serializeText(tags);
        bufferHex += ContentData.serializeNumber(this.price);
...
class Comment extends ContentData {
    ...
    deserialize(buffer, offset) {
        ...
        let varInt = varint.decode(buffer, offset);
        offset += varint.decode.bytes;
        this.description = buffer.slice(offset, offset + varInt).toString('utf8');
        offset += varInt;
        return offset;
    }
}
...
class Trantor {
     ...
     * @param callback
     */
    addTorrent(magnetUri, path, callback) {
        let that = this;
        let torrentOpt = {
            path: path
        };

        this.torrentClient.add(magnetUri, torrentOpt, function (torrent) {
            torrent.files.forEach(function (file) {
                that.seedFile(file.path, function (torrent) {
    ...
    broadcastTorrent(txid, torrent) {
        let that = this;
        that.events.notify('onTorrentDownloaded', 100, txid, torrent);
    ...
    insertAuthor(user, tx, date, callback) {
        ...
        this.database.getAuthor(user.address, function (result) {
            if (result && result.length > 0) {
                ...
                if (user.address === userAddresss) {
                    that.downloadTorrent(tx.hash, user.avatar);
                } else {
                    that.addTorrent(user.avatar, './torrents/user/', function (torrent) {
                ...
                let makeTx = function () {
                    ...
                    if (tasks === 0) {
                        ...
                        that.createDataTransaction(postBuffer, publishAddress, null, function (rawTx) {
                            that.events.notify('onBeforePublish', 10, mediaPost, rawTx);
                        })
                    }
