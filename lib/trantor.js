...
class Trantor {
    ...
    initClients(callback) {
        ...
        setTimeout(function () {
            ...
            OS.run(daemon + ' -usehd -datadir=' + folder, function (result, stderr) {
                console.log('Starting daemon', daemon, result, stderr);
                callCallback();
            });


            File.mkpath(Constants.DATABASE_FILE, true);
            that.database = new Storage(Constants.DATABASE_FILE);
            that.database.init(function () {
                console.log('Database initialized');
                callCallback();
