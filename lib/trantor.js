...
class Storage {
    ...
    getFollowers(userAddress, callback) {
        this.query("SELECT f.follower_address, " +
            "u.* FROM 'Following' f " +
            "LEFT JOIN (SELECT a.*, " +
            "(SELECT count(*) FROM 'Following' f2 WHERE a.address = f2.followed_address AND f2.follower_address = '" + userAddress + "' AND f.type = " + PUBLICATION.TYPE.FOLLOW + ") AS is_following," +
    ...
    getFollowing(userAddress, callback) {
        this.query("SELECT f.followed_address, " +
            "u.* FROM 'Following' f " +
            "LEFT JOIN (SELECT a.*, " +
            "(SELECT count(*) FROM 'Following' f2 WHERE a.address = f2.follower_address AND f2.follower_address = '" + userAddress + "' AND f.type = " + PUBLICATION.TYPE.FOLLOW + ") AS is_following," +
            "(SELECT t.file FROM 'Torrent' t WHERE t.magnet = a.avatar) AS avatarFile FROM 'Author' a) u ON " +
