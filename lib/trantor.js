...
const PUBLICATION = {};
PUBLICATION.START_BLOCK = isDev ? 43300 : 78091;
...
Constants.PID_FILE = Constants.BIN_FOLDER + (Constants.DEBUG ? 'testnet3' + Constants.FILE_SEPARATOR : '') + 'creativecoin.pid';
...
class Trantor {
    constructor(network = NETWORK) {
        ...
        let that = this;
        setTimeout(function () {
            that.torrentClient = new WebTorrent();
            that.torrentClient.on('error', function (err) {
                that.events.notify('onTorrentError', 10, err);
            })
        })
    }
    ...
    checkBinariesExists(callback) {
        ...
        if (!File.exist(Constants.BIN_FOLDER + binaryName)) {
    ...
    explore() {
        let startBlock = this.trantorStorage.getKey('lastExploredBlock', PUBLICATION.START_BLOCK);
                                ...
                                that.client.getBlock(blockHash, function (err, block) {
                                    ...
                                    txIds.forEach(function (txHash) {
                                        ...
                                        let processTx = function (err, rawTx) {
                                            ...
                                            if (err) {
                                                console.error('Error getting tx', txHash, err);
    ...
    stop() {
        ...
        if (File.exist(Constants.PID_FILE)) {
            let pid = File.read(Constants.PID_FILE);
            pid = pid.replace('\n', '');

            OS.run('kill ' + pid, function (error, result, stderr) {
...
/*                    setTimeout(function () {
    ...
    buildDataOutput(data, callback) {
        ...
        let buildOutData = function (dataHex, error) {
            ...
            if (!error) {
                console.log('Final data:', outData.length, outData.toString('hex'));
            ...
            if (spendables.length > 0) {
                that.buildDataOutput(data, function (opReturnData) {
                    ...
                    if (txBuilder.complete) {
                        ...
                        console.error('Tx is incomplete', txBuilder, spendables);
                    }
                });
            } else {
    ...
    register(userAddress, nick, email, web, description, avatar, tags) {
        ...
        setTimeout(function () {
            ...
            that.createTorrent(avatar, destPath, function (torrent) {
                File.write(Constants.TORRENT_DATA_FOLDER + userAddress + '.torrent', torrent.torrentFile);
            ...
            that.client.getNewAddress(function (err, result) {
                ...
                if (publicFile) {
                    ...
                    that.createTorrent(publicFile, destPublicPath, function (publicTorrent) {
                        File.write(Constants.TORRENT_DATA_FOLDER + publishAddress + '.torrent', torrent.torrentFile);
                ...
                if (privateFile) {
                    ...
                    that.createTorrent(privateFile, destPrivatePath, function (privateTorrent) {
                        File.write(Constants.TORRENT_DATA_FOLDER + publishAddress + '-p.torrent', torrent.torrentFile);
