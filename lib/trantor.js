...
class Storage {
    ...
    addPayment(payment, tx, callback) {
        let insertPayment = this.database.prepare('REPLACE INTO Like VALUES (?, ?, ?, ?, ?)');
    ...
    getPayment(userAddress, contentAddress, callback) {
        this.query('SELECT * FOM Payment WHERE content_id = "' + contentAddress + '" AND author = "' + userAddress + '"', callback);
    }
        ...
        setTimeout(function () {
            that.database.getMediaByAddress(contentAddress, function (err, result) {
                if (err) {
                    ...
                    that.createDataTransaction(paymentData, contentAddress, result.price, function (txBuilder, spendables) {
                        that.signTransaction(txBuilder, spendables, function (err, rawTx) {
                            if (err) {
                                that.log(err);
                            } else {
                                let txBuffer = Buffer.from(rawTx, 'hex');
                                that.events.notify('onBeforePayment', 10, txBuilder, txBuffer, paymentData);
                            }
                        })
                    })
                }
