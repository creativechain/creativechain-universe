...
Constants.APP_CONF_FILE = Constants.APP_FOLDER + Constants.FILE_SEPARATOR + 'app' + (Constants.DEBUG ? '' : '_main') + '.conf';
...
class TxOutput {
    ...
    hasData() {
        if (this.hasRawData()) {
            ...
            return dataHex.startsWith(ContentData.serializeNumber(mByte));
        }
...
class Storage {
    ...
    parallelize(callback) {
        this.database.parallelize(callback);
    }
...
class Trantor {
    ...
    prepareConfiguration() {
        ...
        config.setIfNotExist('rpcworkqueue', 10000);
    ...
    explore(startBlock = 0) {
        ...
        let processBlockHash = function () {
            that.client.getBlock(nextBlockHash, function (err, block) {
                //that.log(block);
                if (err) {
                    //Block not found
                    ...
                    txIds.forEach(function (txHash) {
                        ...
                        that.client.getRawTransaction(txHash, function (err, rawTx) {
                            if (err) {
        ...
        this.database.getLastExploredBlock(function (err, result) {
            ...
            that.client.getBlockCount(function (err, result) {
                if (!err) {
                    ...
                    that.client.getBlockHash(startBlock, function (err, blockHash) {
                        if (!err) {
                            ...
                            //Exploration finish
                            that.isExploring = false;
                            that.database.updateLastExploredBlock(++startBlock, function (err, result) {
                                //console.log(err, result);
                            });
                            //that.trantorStorage.setKey('lastExploredBlock', blockHeight);
                            that.events.emit('onExploreFinish', blockCount, startBlock);
                        } else {
                            console.error(err);
                        }
                    })
