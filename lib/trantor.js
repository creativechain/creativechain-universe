...
Constants.START_BLOCK = Constants.DEBUG ? 0 : 100000;
...
class Storage {
     ...
     * @param {string} userAddress
     */
    getMediaByAddress(address, callback, userAddress = null) {
        this.query("SELECT m.*, " +
            "(SELECT count(*) FROM 'Like' l WHERE m.address = l.content_id) AS likes, " +
            "(SELECT count(*) FROM 'Unlike' ul WHERE m.address = ul.content_id) AS unlikes, " +
            "(SELECT count(*) FROM Comment c WHERE m.address = c.content_id) AS comments, " +
...
/*            if (databaseExist) {
     ...
     * @param {number} price
     * @param {string} publicFileHash
     * @param {string} privateFileHash
     * @param {number} publicFileSize
     * @param {number} privateFileSize
     */
    publish(userAddress, publishAddress, title, description, contentType, license, tags, publicTorrent, privateTorrent, price, publicFileHash, privateFileHash, publicFileSize, privateFileSize) {
        let that = this;
        ...
        setTimeout(function () {
            ...
            let hash = privateTorrent && privateTorrent.magnetURI ? privateFileHash : publicFileHash;
