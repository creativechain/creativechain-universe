...
class OS {
    ...
    static getUserDir() {
        if (OS.isLinux() || OS.isMac()) {
            return process.env.HOME + OS.getPathSeparator();
        }

        return process.env.HOMEPATH + OS.getPathSeparator();
    }

    static getInstallDir() {
        if (OS.isLinux() || OS.isMac()) {
            return OS.getUserDir();
        }

        return process.env.APPDATA + OS.getPathSeparator();
    }
    ...
    static getPlatformDir() {
        let platformDir = 'creativechain-platform';
        if (OS.isLinux() || OS.isMac()) {
            platformDir = '.' + platformDir;
        }

        return OS.getInstallDir() + platformDir;
    }
...
Constants.APP_FOLDER = OS.getPlatformDir();
...
class File {
    ...
    static write(path, content, format = 'utf8') {
        ...
        fs.writeFileSync(path, content, format);
    }
...
class Storage {
            ...
            if (err) {
                ...
                let performMigration = function (version) {
                    ...
                    if (File.exist(file)) {
                        ...
                        that.database.run(queries, function (err) {
                            if (!err) {
    ...
    run(query, callback) {
        ...
        let err = null;
        try {
            let stmnt = this.database.prepare(query);
            stmnt.run();
        } catch (e) {
            err = e;
        }
    ...
    insertTorrent(torrent, file, callback) {
        ...
        insertTorrent.run(torrent.infoHash, torrent.magnetURI, file);
        if (callback) {
            callback();
        }
    }
    ...
    getTorrent(torrent, callback) {
        this.select('SELECT * FROM Torrent WHERE hash = ' + torrent, callback);
    }
     ...
     * @param callback
     */
    getBlocked(author, resource, callback) {
                ...
                if (error) {
                    ...
                    that.signTransaction(txBuilder, spendables, function (err, rawTx) {
                        if (err) {
                            ...
                            let txBuffer = Buffer.from(rawTx, 'hex');
                            let tx = creativecoin.Transaction.fromBuffer(txBuffer);
                            that.events.emit('onBeforePublish', txBuffer, mediaPost, txBuilder);
                        }
