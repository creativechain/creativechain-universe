...
class Storage {
     ...
     * @param callback
     */
    insertUserTags(userAddress, tags, callback) {
    ...
    getComments(contentAddress, callback) {
        this.query("SELECT c.*,  u.* FROM Comment c LEFT JOIN (SELECT a.address AS user_address, a.name, a.email, a.web, " +
            ...
            "user_comments, (SELECT count(*) FROM 'Like' l WHERE l.author = a.address) AS user_likes FROM Author a) u " +
    ...
    addLike(like, tx, callback) {
        ...
        insertLike.run(tx.hash, like.version, like.address, like.contentAddress, function (err) {
            if (callback) {
                callback(err);
            }
        });
    ...
    addMedia(media, tx, date, callback) {
        ...
        insertMedia.run(tx.hash, media.version, date, media.userAddress, media.contentAddress, media.type, media.title, media.description, media.contentType, media.tags, media.price, media.publicContent, media.privateContent, function (err) {
            if (callback) {
                callback(err);
            }
        });
    ...
    getAllMedia(callback) {
        this.query("SELECT m.*, (SELECT count(*) FROM 'Like' l WHERE m.txid = l.content_id) AS likes,  (SELECT count(*) " +
            ...
            "a.address) AS user_comments, (SELECT count(*) FROM 'Like' l WHERE l.author = a.address) AS user_likes FROM " +
            "Author a) u ON (u.user_address = m.author) ORDER BY m.creation_date DESC;", callback)
    }
    ...
    getMediaByContentId(contentId, callback) {
        this.query("SELECT m.*, (SELECT count(*) FROM 'Like' l WHERE m.txid = l.content_id) AS likes,  (SELECT count(*) " +
            ...
            "Torrent t2 WHERE t2.magnet = a.avatar) AS avatarFile, (SELECT count(*) FROM Comment c WHERE c.author = " +
            "a.address) AS user_comments, (SELECT count(*) FROM 'Like' l WHERE l.author = a.address) AS user_likes FROM " +
            "Author a) u ON (u.user_address = m.author) WHERE m.txid = '" + contentId + "' ORDER BY m.creation_date DESC;", callback)
    }
    ...
    getMediaByAddress(address, callback) {
        this.query("SELECT m.*, (SELECT count(*) FROM 'Like' l WHERE m.txid = l.content_id) AS likes,  (SELECT count(*) " +
            ...
            "Torrent t2 WHERE t2.magnet = a.avatar) AS avatarFile, (SELECT count(*) FROM Comment c WHERE c.author = " +
            "a.address) AS user_comments, (SELECT count(*) FROM 'Like' l WHERE l.author = a.address) AS user_likes FROM " +
            "Author a) u ON (u.user_address = m.author) WHERE m.address = '" + address + "' ORDER BY m.creation_date DESC;", callback)
    }
...
/*                    setTimeout(function () {
     ...
     * @param {Array} tags
     */
    register(nick, email, web, description, avatar, tags) {
        let that = this;
    ...
    insertAuthor(user, tx, date, callback) {
        let userAddresss = localStorage.getItem('userAddress');
        let that = this;
     ...
     * @param {number} price
     */
    publish(title, description, contentType, license, tags, publicFile, privateFile, price) {
        let that = this;
     ...
     * @param {string} comment
     */
    makeComment(contentAddress, comment) {
        let that = this;
        let userAddress = localStorage.getItem('userAddress');
...
if (module) {
    module.exports = {CoinUri, Currency, UnknownCurrency, FiatCurrency, CryptoCurrency, Eur, Usd, Mxn, Pln, Btc, Crea, Coin,
        ...
        OS, File, Constants, FileStorage, Utils, ContentData, Author, MediaData,Like, Comment, Donation, Following,
        Follow, Unfollow, TxInput, TxOutput, DecodedTransaction, Network, NodeConfiguration, EventHandler, Trantor, Notifications, Globals}
}