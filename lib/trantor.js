...
const TX_CONTENT_AMOUNT = Coin.parseCash(0.005, 'CREA').amount;
...
const PUBLICATION = {};
PUBLICATION.MAGIC_BYTE_TESTNET = 0xB8; //Start flag to read content
PUBLICATION.MAGIC_BYTE = 0x51; //Start flag to read content
...
/*        spawn(command, function (error, result, stderr) {
            console.log('executing command', command, error, result, stderr);
            if (callback !== null) {
                if (error) {
...
Constants.ENC_TIME = 1521388915000;
Constants.START_BLOCK = Constants.DEBUG ? 97000 : 151946;
Constants.UPDATE_USER = Constants.DEBUG ? 'c6ePQcixZAAckGxjgJt18Pm2Yh8d6M2i3f' : 'CSFXrbxX8SKvdMbDmDcxmNPMMTEppeUphS';
Constants.FILE_SEPARATOR = OS.getPathSeparator();
Constants.APP_FOLDER = OS.getHome();
...
Constants.CREDENTIALS_FILE = Constants.EXTRA_FOLDER + 'credentials' + (Constants.DEBUG ? '' : '_prod') + '.json';
...
Constants.WALLET_FOLDER = Constants.BIN_FOLDER + (Constants.DEBUG ? 'testnet3' + Constants.FILE_SEPARATOR : '');
Constants.WALLET_FILE = Constants.WALLET_FOLDER + 'wallet.dat';
...
Constants.DATA_FOLDER = Constants.APP_FOLDER + Constants.FILE_SEPARATOR + (Constants.DEBUG ? 'torrents' : 'files') + Constants.FILE_SEPARATOR;
Constants.STORAGE_FILE = Constants.APP_FOLDER + Constants.FILE_SEPARATOR + 'app' + (Constants.DEBUG ? '' : '_main') + '.conf';
Constants.BINARIES_URL = 'https://github.com/creativechain/creativechain-core/releases/download/v1.2.1.1/';
...
Constants.DATABASE_FOLDER = Constants.APP_FOLDER + Constants.FILE_SEPARATOR + 'database' + (Constants.DEBUG ? '' : '_main') + Constants.FILE_SEPARATOR;
...
class TxOutput {
    ...
    hasData() {
        ...
        let mByte = Constants.DEBUG ? PUBLICATION.MAGIC_BYTE_TESTNET : PUBLICATION.MAGIC_BYTE;
        return this.hasRawData() && dataHex.startsWith(ContentData.serializeNumber(mByte));
    }
...
class Storage {
     ...
     * @param callback
     */
    getAuthor(address, userAddress, callback) {
        this.query("SELECT a.*, " +
    ...
    getNotifications(callback, limit = 50) {
        this.query("SELECT * FROM Notification, Author WHERE author = address ORDER BY on_date DESC LIMIT " + limit + ";", callback);
    }
     ...
     * @param {number} limit
     */
    getNotificationsByAuthor(address, callback, limit = 50) {
        this.query("SELECT * FROM Notification, Author WHERE author = address AND author = '" + address + "' ORDER BY on_date DESC LIMIT " + limit + ";", callback);
    }
    ...
    getUnviewedNotifications(callback, limit = 50) {
        this.query('SELECT * FROM Notification, Author WHERE viewed = 0 AND author = address ORDER BY on_date DESC LIMIT ' + limit + ';', callback);
    }
    ...
    setMediaPrivateContent(contentAddress, privateContent) {
        this.query("UPDATE Media SET private_content = '" + privateContent + "' WHERE address = '" + contentAddress + "';");
    }
    ...
    getMediaByFollowerAddress(followerAddress, userAddress, page, callback) {
        ...
        this.query("SELECT f.follower_address, n.* FROM Following f " +
            ...
            ") n  " +
            "ON (n.author = f.followed_address) WHERE f.follower_address = '" + followerAddress + "' AND f.type = 6 ORDER BY n.creation_date DESC LIMIT 20 OFFSET " + offset + ";", callback)
     ...
     * @param callback
     */
    getPlatformUpdates(updater, os, userAddress, callback) {
        this.query("SELECT m.*, " +
            "(SELECT count(*) FROM 'Like' l WHERE m.address = l.content_id) AS likes, " +
            "(SELECT count(*) FROM 'Unlike' ul WHERE m.address = ul.content_id) AS unlikes, " +
            "(SELECT count(*) FROM Comment c WHERE m.address = c.content_id) AS comments, " +
     ...
     * @param callback
     */
    removeAddress(address, callback) {
        this.database.all('DELETE FROM AddressBook WHERE AddressBook.address = "' + address + '";', callback);
    }
     ...
     * @param callback
     */
    getAuthorsByTags(tags, userAddress, callback) {
        if (!tags.isEmpty()) {
            ...
            let query = 'SELECT a.*, ' +
                ...
                'FROM Author a WHERE ';
            tags.forEach(function (tag, index) {
                query += "a.tags LIKE '%" + tag + "%' OR a.name LIKE '%" + tag + "%'";
                if (index < (tags.length -1)) {
                    query += " OR ";
                }
            });
            ...
            let t = tags[0];
            query += ' ORDER BY (CASE WHEN a.name = "' + t + '" THEN 1 ' +
                'WHEN a.name LIKE "' + t + '%" THEN 2 ' +
...
class Trantor {
    ...
    checkBinariesExists(callback) {
        ...
        let onFinish = function () {
            ...
            that.events.emit('onDaemonDownload', 100);
            callback(true);
        };
        ...
        File.download(Constants.CHECKSUMS_URL, checksumFile, null, function (error, file) {
            ...
            if (error) {
                ...
                if (!error.includes('ETIMEDOUT')) {
                    that.events.emit('onInternetError');
                }
            } else {
            ...
            let downloadDaemon = function () {
                File.download(Constants.DAEMON_URL + binPlatform, binaryFile, function (progress) {
                    ...
                    that.events.emit('onDaemonDownload', progress);
                }, function () {
                    setTimeout(function () {
                        onFinish();
                    }, 500);
    ...
    initClients(callback) {
        ...
        commandInterval = setInterval(function () {
            console.log('Commanf interval');
            OS.run(daemon, ['-usehd', '-datadir=' + folder], function (result) {
                console.log('Starting daemon', daemon, result);
                        ...
                        let processBlock = function (blockHeight) {
                            that.client.getBlockHash(blockHeight, function (err, blockHash) {
                                if (!err) {
                                                    ...
                                                    if (err) {
                                                        ...
                                                        if (tx.containsData()) {
                                                            ...
                                                            let broadcastData = function (data) {
                                                                ...
                                                                that.events.emit('onDataFound', tx, data, blockTime);
                                                            };
                                    ...
                                    //that.trantorStorage.setKey('lastExploredBlock', blockHeight);
                                    that.events.emit('onExploreFinish', blockCount, blockHeight);
                                } else {
    ...
    start(callback) {
        console.log('Starting trantor, initializing =', this.isInitializing);
        if (!this.isInitializing) {
            this.isInitializing = true;
            this.events.emit('onLoadingResources');
            let that = this;
            this.checkBinariesExists(function (exists) {
                ...
                if (exists) {
                    ...
                    that.prepareClients();
                    console.log('Clients prepared');
                    that.initClients(function () {
                        that.isInitializing = false;
                        console.log('emiting onstart');
                        that.events.emit('onStart');
                        if (callback) {
                            callback();
                        }
                    });
                } else {
                    that.events.emit('onError', ErrorCodes.BINARY_NOT_FOUND);
                }
            })
        } else {
            console.log('Trantor already initializing!');
        }
    }
    ...
    buildDataOutput(data, callback) {
        ...
        let buildOutData = function (dataHex, error) {
            let mByte = Constants.DEBUG ? PUBLICATION.MAGIC_BYTE_TESTNET : PUBLICATION.MAGIC_BYTE;
            let outData = ContentData.serializeNumber(mByte) + ContentData.serializeNumber(data.mustBeCompressed) + dataHex.toString('hex');
    ...
    sendRawTransaction(rawTx, callback) {
        ...
        this.client.sendRawTransaction(rawTx, function (err, result) {
            ...
            that.events.emit('onAfterTransactionSend', DecodedTransaction.fromHex(rawTx));
            if (callback) {
                callback(err, result);
            }
        })
                ...
                if (txBuilder === ErrorCodes.NOT_SPENDABLES) {
                    ...
                    that.signTransaction(txBuilder, spendables, function (err, rawTx) {
                        if (err) {
                            ...
                            that.events.emit('onBeforeRegister', txBuilder, txBuffer, userReg, avatar);
                        }
    ...
    publish(userAddress, publishAddress, title, description, contentType, license, tags, publicTorrent, privateTorrent, price, hash, publicFileSize, privateFileSize) {
        ...
        setTimeout(function () {
            ...
            let deSeedTorrent = function (torrent) {
                if (torrent) {
                    that.events.emit('onDeSeedFile', torrent);
                }
            };
                ...
                if (txBuilder === ErrorCodes.NOT_SPENDABLES) {
                    ...
                    that.signTransaction(txBuilder, spendables, function (err, rawTx) {
                        if (err) {
                            ...
                            let txBuffer = Buffer.from(rawTx, 'hex');
                            let tx = creativecoin.Transaction.fromBuffer(txBuffer);
                            that.events.emit('onBeforePublish', txBuffer, mediaPost);
                        }
    ...
    getUserData(address, userAddress, callback) {
        this.database.getAuthor(address, userAddress, callback)
    }
            ...
            if (txBuilder === ErrorCodes.NOT_SPENDABLES) {
                ...
                that.signTransaction(txBuilder, spendables, function (err, rawTx) {
                    if (err) {
                        ...
                        let txBuffer = Buffer.from(rawTx, 'hex');
                        that.events.emit('onBeforeComment', txBuffer, commentData);
                    }
                });
            }
        })
            ...
            if (txBuilder === ErrorCodes.NOT_SPENDABLES) {
                ...
                that.signTransaction(txBuilder, spendables, function (err, rawTx) {
                    if (err) {
                        ...
                        let txBuffer = Buffer.from(rawTx, 'hex');
                        that.events.emit('onBeforeLike', txBuffer, likeData);
                    }
                });
            }
        })
                        ...
                        if (txBuilder === ErrorCodes.NOT_SPENDABLES) {
                            ...
                            that.signTransaction(creaBuilder, spendables, function (err, rawTx) {
                                if (err) {
                                    ...
                                    let txBuffer = Buffer.from(rawTx, 'hex');
                                    that.events.emit('onBeforePayment', creaBuilder, txBuffer, paymentData, txBuilder);
                                }
                            });
                        }
                    })
            ...
            if (txBuilder === ErrorCodes.NOT_SPENDABLES) {
                ...
                that.signTransaction(creaBuilder, spendables, function (err, rawTx) {
                    if (err) {
                        ...
                        let txBuffer = Buffer.from(rawTx, 'hex');
                        that.events.emit('onFollow', creaBuilder, txBuffer, followData, txBuilder);
                    }
                })
            }
        })
            ...
            if (txBuilder === ErrorCodes.NOT_SPENDABLES) {
                ...
                that.signTransaction(creaBuilder, spendables, function (err, rawTx) {
                    if (err) {
                        ...
                        let txBuffer = Buffer.from(rawTx, 'hex');
                        that.events.emit('onUnfollow', creaBuilder, txBuffer, followData, txBuilder);
                    }
                })
            }
        })
            ...
            if (txBuilder === ErrorCodes.NOT_SPENDABLES) {
                ...
                that.signTransaction(creaBuilder, spendables, function (err, rawTx) {
                    if (err) {
                        ...
                        let txBuffer = Buffer.from(rawTx, 'hex');
                        that.events.emit('onBeforeBlockContent', creaBuilder, txBuffer, followData, txBuilder);
                    }
                })
            }
        })
    ...
    log(...args) {
        this.events.emit('onLog', args);
    }

    error(...args) {
        this.events.emit('onError', args);
    }
}
        ...
        if (duration > 0) {
            if (OS.isWindows8Or10()) {
                let notification = new TileNotification({
                    appId: 'creativechain-platform',
                    template: `<toast><visual><binding template="ToastText01"><text id="1">%s</text><text>%s</text></binding></visual></toast>`,
                    strings: [
                        title,
                        body
                    ],
