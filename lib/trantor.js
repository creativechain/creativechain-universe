...
Constants.START_BLOCK = Constants.DEBUG ? 53000 : 82200;
...
class Storage {
    ...
    getAllMedia(callback) {
        this.query("SELECT m.*, (SELECT count(*) FROM 'Like' l WHERE m.address = l.content_id) AS likes,  (SELECT count(*) " +
            ...
            "m.public_content) AS featured_image, (SELECT t2.file FROM Torrent t2 WHERE t2.magnet = m.private_content) AS " +
            "private_file, u.* FROM Media m LEFT JOIN (SELECT a.address AS user_address, a.name, " +
    ...
    checkBinariesExists(callback) {
        ...
        File.download(Constants.CHECKSUMS_URL, checksumFile, null, function () {
            ...
            if (checksum) {
                ...
                that.log('Checksum not found!!', content);
                if (!File.exist(binaryFile)) {
                    downloadDaemon();
                }
            }
                                ...
                                that.client.getBlock(blockHash, function (err, block) {
                                    ...
                                    txIds.forEach(function (txHash) {
                                        ...
                                        let processTx = function (err, rawTx) {
                                            ...
                                            if (err) {
                                                that.log('Error getting tx', txHash, err);
            ...
            if (spendables.length > 0) {
                that.buildDataOutput(data, function (opReturnData) {
                    ...
                    if (txBuilder.complete) {
                        ...
                        that.log('Tx is incomplete', txBuilder, spendables);
                    }
                });
            } else {
                that.log('Not found spendables for this data', err, spendables);
            }
...
class Torrents {
    ...
    seedFile(file, callback) {
        ...
        if (this.containsTorrent(file)) {
            let torrent = this.getTorrent(file);
            onTorrent(torrent);
        } else {
    ...
    downloadTorrent(contentAddress, magnet, privateContent = false) {
        ...
        if (magnet) {
            ...
            if (!this.containsTorrent(magnet)) {
                this.torrentClient.add(magnet, {path: path}, function (torrent) {
                    torrent.on('done', function () {
                        ...
                        that.trantor.database.putTorrent(tHash, torrent, file, function (err) {
                            that.trantor.log(err);
                        });
                    })
                })
            } else {
                let torrent = this.getTorrent(magnet);
                this.trantor.events.notify('onTorrentDownloaded', 100, contentAddress, torrent);
            }
