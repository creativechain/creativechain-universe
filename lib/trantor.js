...
class Storage {
    ...
    addMedia(media, tx, date, callback) {
        ...
        insertMedia.run(tx.hash, media.version, date, media.contentAddress, media.type, media.title, media.description, media.privateContent, media.userAddress, function (err) {
            if (callback) {
                callback(err);
            }
        });
...
class Trantor {
    constructor(network = NETWORK) {
        ...
        this.onBeforeTransactionSend = null;
        this.onAfterTransactionSend = null;
        this.onBeforePublish = null;
        this.onAfterPublish = null;
    }
    ...
    explore(startBlock = 0) {
        ...
        setTimeout(function () {
            if (that.onExploreStart) {
                that.onExploreStart(startBlock);
            }
                ...
                let processBlock = function (blockHeight) {
                    that.client.getBlockHash(blockHeight, function (err, blockHash) {
                        if (!err) {
                                    ...
                                    that.getRawTransaction(txHash, function (err, rawTx) {
                                        ...
                                        if (tx.containsData()) {
                                            ...
                                            let broadcastData = function (data) {
                                                if (that.onDataFound) {
                                                    that.onDataFound(tx, data, blockTime);
                                                }
                                            };
                                            let data = tx.getData();
                            ...
                            that.onExploreFinish(blockCount);
                        } else {
    ...
    broadcastOnBeforeTransactionSend(txBuilder, spendables) {
        if (this.onBeforeTransactionSend) {
            let that = this;
            setTimeout(function () {
                that.onBeforeTransactionSend(txBuilder, spendables);
            }, 10);
        }
    }
    ...
    broadcastOnAfterTransactionSend(tx) {
        if (this.onAfterTransactionSend) {
            let that = this;
            setTimeout(function () {
                that.onAfterTransactionSend(tx);
            }, 10);
        }
    }
    ...
    broadcastOnBeforePublish(post, rawTx) {
        if (this.onBeforePublish) {
            let that = this;
            setTimeout(function () {
                that.onBeforePublish(post, rawTx);
            }, 10)
        }
    }
        ...
        setTimeout(function () {
            that.getNewAddress(function (err, result) {
                ...
                that.createTorrent(publicFile, destPublicPath, function (publicTorrent) {
                    ...
                    let makeTx = function (pubTorrent, prvTorrent) {
                    ...
                    if (privateFile) {
                        ...
                        that.createTorrent(privateFile, destPrivatePath, function (privateTorrent) {
                            console.log('Private Torrent created!', publicTorrent);
                            makeTx(publicTorrent, privateTorrent)
                        })
                    } else {
                        makeTx(publicTorrent);
                    }
                })
            })
        }, 10)
