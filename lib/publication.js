        ...
        ok: {
            ...
            onclick: function () {
                ...
                trantor.sendRawTransaction(txBuffer.toString('hex'), function (err, result) {
                    if (!err) {
                        trantor.events.emit('onAfterRegister', 10, txBuffer, userReg, torrent);
                        modal.hide();
                        editingArticle = null;
                    } else {
                        modal.hide();
...
trantor.events.on('onBeforePublish', function (txBuffer, mediaPost) {
    trantor.client.sendRawTransaction(txBuffer.toString('hex'), function (err, result) {
        if (err) {
            ...
            trantor.events.emit('onAfterPublish', 10, tx, mediaPost);
        }
    })
});
...
function register() {
    ...
    getUserAddress(function (userAddress) {
        ...
        let onRegister = function (avatarData) {
            trantor.register(userAddress, username, email, web, description, avatarData, tags);
        };
