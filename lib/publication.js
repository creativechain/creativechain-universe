        ...
        getUserAddress(function (userAddress) {
            ...
            let onMakePublish = function () {
                ...
                if (featuredImage) {
                    ...
                    trantor.ipfsrunner.send('createFile', featuredImage, function (error, file, ipfsData) {
                        console.log('Public Torrent created!', ipfsData);
                        let tHash = Utils.makeHash(ipfsData.CID);
                        trantor.dbrunner.send('putTorrent', tHash, ipfsData.CID, ipfsData.path, file);
                        pubTorrent = ipfsData;
                        makePost();
                    })
                }
                ...
                if (privateFile) {
                    ...
                    if (privateFile === featuredImage) {
                        ...
                        trantor.ipfsrunner.send('createFile', privateFile, function (error, file, ipfsData) {
...
function register() {
    ...
    getUserAddress(function (userAddress) {
        ...
        if (profileImage && profileImage.length > 0) {
            ...
            trantor.ipfsrunner.send('createFile', profileImage, function (error, file, ipfsData) {
                console.log('Register torrent', ipfsData);
                onRegister(ipfsData);
                let tHash = Utils.makeHash(ipfsData.CID);
                trantor.dbrunner.send('putTorrent', tHash, ipfsData.CID, destPath, file);
            })
        } else {
            onRegister(null);
        }
