...
trantor.on('core.register.build', function (txBuilt, userReg, avatarCID) {

    let fee = Coin.parseCash(txBuilt.fee, 'CREA');
        ...
        ok: {
            ...
            onclick: function () {
                ...
                signAndSendTx(txBuilt.hex, function (err, txid, signedTx) {
                    if (!err) {
                        trantor.emit('core.register.sent', signedTx, userReg, avatarCID);
                        modal.hide();
                        editingArticle = null;
                    } else {
...
trantor.on('core.publication.build', function (txBuilt, mediaPost) {
