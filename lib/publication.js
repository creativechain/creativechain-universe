...
trantor.events.subscribe('onBeforePublish', 'public', function (args) {
    ...
    trantor.client.sendRawTransaction(txBuffer.toString('hex'), function (err, result) {
        if (err) {
            ...
            Notifications.notify(lang.ContentPublished, lang.ContentPublishedSucessfully, './assets/img/publication1.png', 5);
...
function setUserData() {
    ...
    trantor.getUserData(userAddress, function (err, results) {
        if (results && results.length > 0) {
            ...
            let buzz = BUZZ.getBuzz(user.likes, 0);
...
function publishContent() {
    ...
    let userAddress = localStorage.getItem('userAddress');
    trantor.publish(userAddress, title, description, contentType, license, publishTags, featuredImage, privateFile, price)
}
...
function register() {
    ...
    let userAddress = localStorage.getItem('userAddress');
    trantor.register(userAddress, username, email, web, description, profileImage, tags);
}
