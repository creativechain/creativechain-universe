...
trantor.events.subscribe('onBeforeComment', 'main', function (args) {
    ...
    trantor.client.sendRawTransaction(txBuffer.toString('hex'), function (err, result) {
        if (err) {
            ...
            Notifications.notify(lang.CommentPublished, lang.CommentPublishedSucessfully, './assets/img/publication1.png', 5);
            let tx = DecodedTransaction.fromHex(txBuffer.toString('hex'));
...
function prepareArticle(address) {
    ...
    trantor.database.getMediaByAddress(address, function (err, result) {
        ...
        let userAvatar = resolveAvatar(article.avatarFile, article.author, 50);
        $('#article-featured-image').attr('src', article.featured_image);
        $('#article-comment-avatar').attr('src', userAvatar);
        $('#article-author-avatar').attr('src', userAvatar);
        $('#article-author-name').html(article.name);
        $('#article-author-web').html(article.web || article.user_description);
        ...
        $('#article-format').html(article.content_type);
        $('#article-date').html(new Date(article.creation_date).toLocaleString());
        $('#article-likes').html(article.likes + ' ' + lang.Likes);
        $('#article-comments').html(article.comments + ' ' + lang.Comments);

        let buzz = BUZZ.getBuzz(article.user_likes, 0);
        ...
        $('#article-comment-button').attr('onclick', "publishComment('" + article.address + "')");
        $('#article-comment').val('');
...
function loadComment(contentAddress) {
...
function publishComment(contentAddress) {
    ...
    if (comment && comment.length > 0) {
        ...
        trantor.makeComment(contentAddress, comment);
    }
}
