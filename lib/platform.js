...
function loadMediaItem(data, userAddress, prepend = false) {
    ...
    articleLoader.load(R.ELEMENT.ITEM.PUBLICATION, function () {
        if (!data.avatar_file) {
...
function loadComments(contentAddress) {
    trantor.database.getComments(contentAddress, function (err, comments) {
        if (err) {
            ...
            if (!commentVueList) {
                commentVueList = new Vue({
                    el: '#article-comment-list',
                    data: {
                        comments: comments
                    },
                    methods: {
                        resolveAvatar: resolveAvatar,
                        getBuzz: function(likes, comments, publications, actions = 0) {
                            return BUZZ.getBuzz(likes, comments, publications, actions);
                        },
                ...
                commentVueList.$data.comments = comments;
            }
        }
    })
...
function downloadPrivateFile(contentAddress, privateFile) {
    fileClient.downloadFile(contentAddress, privateFile, function (torrent, file, contentAddress) {
        let tHash = Utils.makeHash(torrent.magnetURI);
        trantor.database.putTorrent(tHash, torrent.magnetURI, torrent.path, file);
        trantor.events.emit('onTorrentDownloaded', contentAddress, torrent);
    }, true);
