...
trantor.events.subscribe('onBeforeTransactionSend', 'index', function (args) {
    ...
    let fee = Coin.parseCash(txBuilder.txFee, 'CREA').toFriendlyString();
    totalOut = Coin.parseCash(totalOut, 'CREA').toFriendlyString();

    let total = txBuilder.txFee + totalOut;
    total = Coin.parseCash(total, 'CREA').toFriendlyString();

    let txMessage = String.format(lanng,TxMessage, totalOut, fee, total);
    modal.alert({
        message: txMessage,
        ok: {
            text: lang.Send,
...
trantor.events.subscribe('onBeforePayment', 'main', function (args) {
    ...
    let totalAmount = Coin.parseCash(totalOutAmount.value + fee.value, 'CREA');
            ...
            getUserAddress(function (userAddress) {
                trantor.database.getFollowingData(userAddress, data.author, PUBLICATION.TYPE.FOLLOW, function (err, result) {
                    if (err) {
                        ...
                        $('#content-item-tooltip-follow-' + address).html(lang.Following).mouseover(function () {
                            $(this).html(lang.Unfollow).click(function () {
                                unFollowUser(data.author);
                            });
                        }).mouseout(function () {
                            $(this).html(lang.Following);
                        })
                    } else {
                        $('#content-item-tooltip-follow-' + address).html(lang.Follow).click(function () {
                            followUser(data.author);
                        })
                    }
                });
            });
...
function loadMediaItem(data, userAddress, prepend = false) {
    ...
    articleLoader.load('./elements/content-item.html', function () {
        ...
        if (userAddress === data.author) {
            $('#content-item-tooltip-follow-').addClass('hidden')
                .attr('id', 'content-item-tooltip-follow-' + data.address);
        } else {
            trantor.database.getFollowingData(userAddress, data.author, PUBLICATION.TYPE.FOLLOW, function (err, result) {
        ...
        $('#content-item-tooltip-avatar-').attr('src', avatar).attr('id', 'content-item-tooltip-avatar-' + data.address);
    ...
    trantor.database.getMediaByAddress(address, function (err, result) {
        ...
        getUserAddress(function (userAddress) {
            ...
            if (userAddress === preparedArticle.author) {
                $('#article-follow').addClass('hidden');
            } else {
                $('#article-follow').removeClass('hidden');
                trantor.database.getFollowingData(userAddress, preparedArticle.author, PUBLICATION.TYPE.FOLLOW, function (err, result) {
                    if (err) {
                        console.error(err);
                    } else if (result.length > 0) {
                        $('#article-follow').html(lang.Following).mouseover(function () {
                            $(this).html(lang.Unfollow).click(function () {
                                unFollowUser(preparedArticle.author);
                            });
                        }).mouseout(function () {
                        ...
                        $('#article-follow').html(lang.Follow).click(function () {
                            followUser(preparedArticle.author);
                        })
                    }
                });
...
function makeLike() {
    if (!preparedArticle.user_liked) {
        ...
        let likeAmount = Coin.parseCash(appStorage.getKey('action-amount'), 'CREA').value;
        getUserAddress(function (userAddress) {
