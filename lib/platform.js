            ...
            getUserAddress(function (userAddress) {
                ...
                trantor.database.getFollowingData(userAddress, data.author, PUBLICATION.TYPE.BLOCK, function (err, result) {
                    if (err) {
                        ...
                        trantor.database.getFollowingData(userAddress, data.address, PUBLICATION.TYPE.BLOCK, function (err, result) {
                            if (err) {
                                console.error(err);
                            } else if (result.length <= 0) {
                                processNotBlock();
                ...
                trantor.database.addFollowing(data, tx, blockTime, function (err) {
                    if (err) {
                        ...
                        if (data.type === PUBLICATION.TYPE.UNFOLLOW) {
                            ...
                            trantor.database.getMediaByAddress(data.followedAddress, userAddress, function (err, result) {
                                if (err) {
                                    console.error(err)
                                } else if (result.length > 0) {
...
trantor.events.on('onBeforeComment', function (txBuffer, comment) {
    ...
    sendTransaction(txBuffer.toString('hex'), walletPassword, function (err, result) {
        if (err) {
            ...
            trantor.events.emit('onNotification', lang.CommentPublished, lang.CommentPublishedSuccessfully, './assets/img/publications1.png', 5);
...
trantor.events.on('onAfterTransactionSend',function (tx) {
    trantor.events.emit('onNotification', lang['TransactionSend'], tx.hash, './assets/img/notification/wallet.png', 2);
});
        ...
        if (pendingContent[contentAddress] && pendingContent[contentAddress].pending) {
            trantor.database.getMediaByAddress(contentAddress, userAddress, function (err, result) {
                if (err) {
                    ...
                    trantor.events.emit('onNotification', lang.DownloadFinished, body, './assets/img/notifications.png', 10);
                    ...
                    if (err) {
                        ...
                        trantor.database.addPayment(payment, dTx, function (err, result) {
                            if (err) {
                                ...
                                trantor.events.emit('onNotification', lang.Payment, lang.PaymentSent, './assets/img/notification/wallet.png', 10);
                                getUserAddress(function (userAddress) {
...
function updateMediaItem(address, userAddress, forceAdd = true) {
    trantor.database.getMediaByAddress(address, userAddress, function (err, result) {
        ...
        if (result.length > 0) {
            ...
            if (added) {
                if (!data.featured_image || !File.exist(data.featured_image) ||  File.fileInfo(data.featured_image).size === 0) {
                    downloadPublicFile(data.address, data.public_content)
                }

                if (!data.avatarFile || !File.exist(data.avatarFile) || File.fileInfo(data.avatarFile).size === 0) {
...
function makePayment() {
    ...
    if (mustBeDownloadContent()) {
            ...
            trantor.database.getPayment(userAddress, preparedArticle.address, function (err, result) {
                if (err) {
                    ...
                    if (result[0]) {
                        ...
                        trantor.events.emit('onNotification', lang.Files, lang.DownloadFileStart, './assets/img/wallet/icon-receive.png', 5);
                    } else {
        ...
        dialog.showSaveDialog(null, {
            ...
            if (fileName) {
                trantor.events.emit('onNotification', lang.Files, lang.CopyingFile, './assets/img/wallet/icon-receive.png', 5);
                setTimeout(function () {
                    File.cp(privateFile, fileName);
                    let notifBody = String.format(lang.FileCopied, fileName);
                    trantor.events.emit('onNotification', lang.Files, notifBody, './assets/img/wallet/icon-receive.png', 10);
                }, 10)
            }
        })
    }
