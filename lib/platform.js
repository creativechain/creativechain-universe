    ...
    trantor.insertComment(comment, tx, new Date().getTime(), function (err) {
        if (err) {
            ...
            setTimeout(function () {
                ...
                updateItem(comment.contentAddress, comment.author);
            }, 200);
    ...
    trantor.database.addLike(like, tx, function (err) {
        if (err) {
            ...
            setTimeout(function () {
                ...
                updateProfileMediaItem(like.contentAddress, like.author);
                    ...
                    trantor.database.getMediaByAuthor(data.followedAddress, data.followerAddress, function (err, result) {
                        if (err) {
                            ...
                            result.forEach(function (media) {
                                updateItem(media.address, data.followerAddress);
...
function prepareArticle(address, userAddress) {
    ...
    trantor.database.getMediaByAddress(address, userAddress, function (err, result) {
        ...
        trantor.database.getComments(address, function (err, comments) {
            if (err) {
                console.error(err);
            } else {
