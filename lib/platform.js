...
trantor.events.on('onDataFound', function (tx, data, blockTime) {
    ...
    switch (data.type) {
        case PUBLICATION.TYPE.USER:
            fileClient.downloadFile(data.address, data.avatar, function (torrent, file, contentAddress) {
                        ...
                        trantor.insertMedia(data, tx, blockTime, function (err, result) {
                            if (err) {
                                ...
                                setTimeout(function () {
                                    ...
                                    if (data.author === Constants.WALLET_FOLDER.UPDATE_USER) {
                                        checkUpdate(data.contentAddress, data.title);
                                        console.log(data.userAddress, data.type, data.contentAddress, blockTime);
...
trantor.events.on('onBeforeLike', function (args) {
    let txBuffer = args[0];
    let like = args[1];
 ...
 */
trantor.events.on('onAfterTransactionSend',function (args) {
    let tx = args[0];
    Notifications.notify(lang['TransactionSend'], tx.hash, './assets/img/notification/wallet.png', 2);
});

trantor.events.on('onTorrentDownloaded', function (address, torrent) {

    getUserAddress(function (userAddress) {
        updateMediaItemView(address, userAddress, false);
        updatePreparedArticle(address);
    });

    if (pendingContent[address] && pendingContent[address].pending) {
...
trantor.events.on('onFollow', function (creaBuilder, txBuffer, data, txBiulder) {
    let dTx = DecodedTransaction.fromHex(txBuffer.toString('hex'));
    trantor.client.sendRawTransaction(txBuffer.toString('hex'), function (err, result) {
        if (err) {
            console.error(err);
                    ...
                    trantor.database.getMediaByAuthor(data.followedAddress, data.followerAddress, function (err, result) {
                        if (err) {
                            ...
                            result.forEach(function (media) {
                                updateMediaItem(media.address, data.followerAddress);
                                updateProfileMediaItem(media.address, data.followerAddress);
                            });
                        }
                    });
...
function updateMediaItemView(mediaAddress, userAddress, forceAdd = true) {
...
function downloadAuthorAvatar(authorAddress, avatarMagnet) {
    fileClient.downloadFile(authorAddress, avatarMagnet, function (torrent, file, address) {
        ...
        //trantor.events.emit('onTorrentDownloaded', 100, address, torrent);
                ...
                trantor.database.getMediaByAuthor(authorAddress, userAddress, function (err, result) {
                    if (err) {
                        ...
                        result.forEach(function (data) {
                            updateMediaItem(data.address, userAddress)
...
function showMediaResults(results, userAddress, add = false) {
    ...
    results.forEach(function (data) {
        ...
        let followButton = $('#content-item-tooltip-follow-' + data.address);
        if (userAddress === data.author) {
...
function loadUserFollowed(data, prepend = false) {
    ...
    //TODO This will be load to followers list in profile view
    followedLoader.load(R.ELEMENT.ITEM.FOLLOWING, function () {
        ...
        if (!articleVue) {
            articleVue = new Vue({
                ...
                data: {
                    ...
                    license: license,
                    lang: lang,
                    buzz: BUZZ.getBuzz(preparedArticle.user_likes, preparedArticle.user_comments, preparedArticle.user_publications),
                    userAddress: userAddress,
                ...
                methods: {
                    resolveAvatar: resolveAvatar,
                    getBuzz: function (likes, comments, publications, actions) {
                        return BUZZ.getBuzz(likes, comments, publications, actions);
                    },
            ...
            articleVue.$data.buzz = BUZZ.getBuzz(preparedArticle.user_likes, preparedArticle.user_comments, preparedArticle.user_publications);
...
function downloadPrivateContent(content, paymentId) {
    if (content.creation_date < Constants.ENC_TIME) {
        ...
        decryptContent(content.address, content.private_content, paymentId, function (decryptedContent) {
