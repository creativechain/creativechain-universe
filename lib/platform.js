...
function loadUserFollows(data, prepend = false) {
    followedLoader.load('./elements/following-item.html', function () {
        ...
        $('#followed-').attr('onclick', "onLoadUser('" + data.address + "');")
            .attr('id', 'following-' + data.address);
        let avatar = resolveAvatar(data.avatarFile, data.address);
        $('#followed-avatar-').attr('src', avatar).attr('id', 'follower-avatar-' + data.address);
        $('#followed-name-').html(data.name || lang.Anonymous).attr('id', 'follower-name-' + data.address);
        $('#followed-description-').html(data.description || '-').attr('id', 'follower-name-' + data.address);
