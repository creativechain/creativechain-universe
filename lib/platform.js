                ...
                trantor.database.addFollowing(data, tx, blockTime, function (err) {
                    if (err) {
                        ...
                        if (data.type === PUBLICATION.TYPE.UNFOLLOW) {
                            ...
                            trantor.database.getAuthor(data.followedAddress, function(err, result) {
                                if (err) {
                                    console.error(err)
                                } else if (result.length > 0) {
                                    onDelete(deleteUser);
...
trantor.events.subscribe('onTorrentDownloaded', 'main', function (args) {
    ...
    if (paymentAddresses[address] && paymentAddresses[address].pending) {
        ...
        let body = String.format(DownloadFinishedBody, pendingDownloads[address].title);
...
function prependItem(txId) {
    if (!onSearch) {
        getUserAddress(function (userAddress) {
            trantor.database.getMediaByContentId(txId, userAddress, function (err, result) {
                if (err) {
                    console.error(err);
                } else if (result.length > 0) {
                    result = result[0];
...
function downloadAuthorAvatar(authorAddress, avatarMagnet) {
    torrentClient.downloadTorrent(authorAddress, avatarMagnet, function (torrent, file, address) {
        ...
        setTimeout(function () {
            getUserAddress(function (userAddress) {
                trantor.database.getMediaByAuthor(authorAddress, function (err, result) {
                    if (err) {
                        console.error(err);
                    } else if (result.length > 0) {
...
function loadAllMedia() {
    if (!onSearch) {
        if (mustReloadContent) {
            getUserAddress(function (userAddress) {
                trantor.database.getAllMedia(userAddress, function (err, result) {
                    if (err) {
                        console.error(err);
                    } else {
...
function loadFollowingMedia() {
    getUserAddress(function (userAddress) {
        trantor.database.getMediaByFollowerAddress(userAddress, userAddress, function (err, result) {
            if (err) {
                console.error(err);
            } else {
                console.log(result);
...
function loadUserFollows(data, prepend = false) {
    ...
    followedLoader.load('./elements/following-item.html', function () {
        ...
        $('#followed-avatar-').attr('src', avatar).attr('id', 'followed-avatar-' + data.followed_address);
...
function prepareArticle(address, userAddress) {
    ...
    trantor.database.getMediaByAddress(address, userAddress, function (err, result) {
        ...
        makeLikeButton('article-like', preparedArticle.user_liked);
