                        ...
                        if (err) {
                            ...
                            trantor.dbrunner.getAuthor(data.userAddress, userAddress, function (err, result) {
                                if (err) {
                                    ...
                                    let author = result[0];
                                    mustReloadContent = page === PAGES.FOLLOWING || !!author.user_following;
...
function downloadAuthorAvatar(authorAddress, avatarMagnet) {
    if (avatarMagnet && avatarMagnet.length > 0) {
        ...
        trantor.ipfsrunner.send('downloadFile', authorAddress, avatarMagnet, false, function (data, file, address) {
...
function downloadPrivateFile(contentAddress, privateContent) {
    ...
    trantor.ipfsrunner.send('downloadFile', contentAddress, privateContent, true, function (data, file, contentAddress) {
