...
trantor.events.subscribe('onDataFound', 'main', function (args) {
    ...
    switch (data.type) {
        case PUBLICATION.TYPE.USER:
            try {
                torrentClient.downloadTorrent(data.address, data.avatar, function (torrent, file) {
                    let tHash = Utils.makeHash(torrent.magnetURI);
                    trantor.database.putTorrent(tHash, torrent, file);
                });
            } catch (err) {
                console.log(err);
            }
        ...
        case PUBLICATION.TYPE.CONTENT:
            if (data.publicContent && data.publicContent.length > 0) {
                torrentClient.downloadTorrent(data.contentAddress, data.publicContent, function (torrent, file) {
                    let tHash = Utils.makeHash(torrent.magnetURI);
                    trantor.database.putTorrent(tHash, torrent, file);
                });
            }
                    ...
                    if (err) {
                        ...
                        trantor.database.addPayment(payment, dTx, function (err, result) {
                            if (err) {
                                ...
                                trantor.database.getMediaByAddress(payment.content_id, function (err, result) {
                                    if (err) {
                                        console.log(err);
                                    } else {
                                        result = result[0];
...
function updateItem(address) {
    trantor.database.getMediaByAddress(address, function (err, result) {
        ...
        $('#content-item-image-' + address).attr('src', featuredImage.image);
...
function loadMediaItem(data, prepend = false) {
    ...
    articleLoader.load('./elements/content-item.html', function () {
        ...
        if (!data.featured_image) {
            ...
            torrentClient.downloadTorrent(data.address, data.public_content, function (torrent, file) {
                let tHash = Utils.makeHash(torrent.magnetURI);
        ...
        getUserAddress(function (userAddress) {
            trantor.database.getFollowingData(userAddress, data.author, PUBLICATION.TYPE.FOLLOW, function (err, result) {
                if (err) {
                    ...
                    $('#content-item-tooltip-follow-').click(function () {
...
function downloadPrivateFile(contentAddress, privateFile) {
    torrentClient.downloadTorrent(contentAddress, privateFile, function (torrent, file) {
        let tHash = Utils.makeHash(torrent.magnetURI);
