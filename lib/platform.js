...
trantor.on('core.data', function (tx, data, blockTime) {
    ...
    switch (data.type) {
        case TrantorConstants.TYPE.USER:
            trantor.dbrunner.send('addAuthor', data, tx, blockTime, function () {
                setTimeout(function () {
        ...
        case TrantorConstants.TYPE.CONTENT:
            getUserAddress(function (userAddress) {
                ...
                let processNotBlock = function () {
                            ...
                            setTimeout(function () {
                                ...
                                if (data.userAddress === CONSTANTS.UPDATE_USER) {
                                    ...
                                    if (checkUpdate(data.contentAddress, data.title)) {
                                        trantor.dbrunner.send('insertNotification', data.userAddress, data.type, data.contentAddress, blockTime);
                    ...
                    trantor.dbrunner.send('getFollowingData', userAddress, data.author, TrantorConstants.TYPE.FOLLOW, function (err, result) {
                        if (err) {
                            console.error(err);
                ...
                trantor.dbrunner.send('getFollowingData', userAddress, data.author, TrantorConstants.TYPE.FOLLOW, function (err, result) {
                    if (err) {
                        ...
                        trantor.dbrunner.send('getFollowingData', userAddress, data.address, TrantorConstants.TYPE.FOLLOW, function (err, result) {
                            if (err) {
                                console.error(err);
                            } else if (result.length <= 0) {
                                processNotBlock();
        ...
        case TrantorConstants.TYPE.COMMENT:
            getUserAddress(function (userAddress) {
                ...
                trantor.dbrunner.send('getMediaByAddress', data.contentAddress, userAddress, function (err, result) {
                    if (err) {
                        console.error(err);
                    } else if (result.length > 0) {
                        let media = result[0];
                        ...
                        if (media.author === userAddress && userAddress !== data.author) {
                            trantor.dbrunner.send('insertNotification', data.author, data.type, data.contentAddress, blockTime);
                            loadAllNotifications();
                        }
                    }
                });
        ...
        case TrantorConstants.TYPE.LIKE:
            ...
            getUserAddress(function (userAddress) {
                trantor.dbrunner.send('addLike', data, tx, function () {
                    setTimeout(function () {
                        updateMediaItemView(data.contentAddress, userAddress);
                        updateAuthor(data.author);
                    }, 200);
        ...
        case TrantorConstants.TYPE.UNLIKE:
            getUserAddress(function (userAddress) {
                trantor.dbrunner.send('addUnlike', data, tx, function () {
                    //TODO: SHOW UNLIKES ?
                });
            });
            break;
        case TrantorConstants.TYPE.PAYMENT:
            getUserAddress(function (userAddress) {
                trantor.dbrunner.send('addPayment', data, tx, function () {
                    setTimeout(function () {
                        updateMediaItemView(data.contentAddress, userAddress);
                        updateAuthor(data.author);
                    }, 200);
        ...
        case TrantorConstants.TYPE.BLOCK:
            getUserAddress(function (userAddress) {
                trantor.dbrunner.send('addFollowing', data, tx, blockTime, function () {
                    if (data.type === TrantorConstants.TYPE.UNFOLLOW) {
                        trantor.dbrunner.send('removeFollowing', data);
                        ...
                        let onDelete = function (deletion) {
                            if (deletion === deleteMedia) {
                                trantor.dbrunner.send('removeMedia', data.followedAddress);
                            } else if (deletion === deleteUser) {
                                trantor.dbrunner.send('removeMediaByAuthor', data.followedAddress);
                            }
                        };

                        trantor.dbrunner.send('getMediaByAddress', data.followedAddress, userAddress, function (err, result) {
                        if (err) {
                            console.error(err)
                        } else if (result.length > 0) {
                ...
                trantor.dbrunner.send('getAuthor', data.followedAddress, userAddress, function(err, result) {
                if (err) {
                    console.error(err)
                } else if (result.length > 0) {
                    onDelete(deleteUser);
                ...
                if (data.type === TrantorConstants.TYPE.FOLLOW) {
                    if (data.followedAddress === userAddress) {
                        trantor.dbrunner.send('insertNotification', data.followerAddress, data.type, data.followedAddress, blockTime);
                        setTimeout(function () {
                            trantor.dbrunner.send('getFollowingData', userAddress, data.followedAddress, TrantorConstants.TYPE.FOLLOW, function (err, result) {
                                if (err) {
                                    console.error(err)
                                } else if (result.length > 0) {
                                    let data = result[0];
...
trantor.on('core.like.sent', function (tx, like) {
    ...
    trantor.dbrunner.send('addLike', like, tx, function (err) {
        if (err) {
            console.error(err);
        } else {
            setTimeout(function () {
...
trantor.on('core.file.ready', function (contentAddress, torrent) {
    ...
    getUserAddress(function (userAddress) {
        ...
        if (pendingContent[contentAddress] && pendingContent[contentAddress].pending) {
            trantor.dbrunner.send('getMediaByAddress', contentAddress, userAddress, function (err, result) {
                if (err) {
                    console.error(err);
                } else if (result.length > 0) {
                    result = result[0];
            ...
            onclick: function () {
                sendTransaction(txBuffer.toString('hex'), walletPassword, function (err, txid) {
                    if (err) {
                        ...
                        console.log(txid);
                        trantor.dbrunner.send('addPayment', payment, dTx, function (err, result) {
                            if (err) {
                                console.error(err);
                            } else {
                                ...
                                getUserAddress(function (userAddress) {
                                    ...
                                    trantor.dbrunner.send('getMediaByAddress', payment.contentAddress, userAddress, function (err, medias) {
                                        if (err) {
                                            console.log(err);
                                        } else {
                                            let m = medias[0];
...
trantor.on('core.follow.build', function (creaBuilder, txBuffer, data, txBuilder) {
    ...
    sendTransaction(txBuffer.toString('hex'), walletPassword, function (err, result) {
        if (err) {
            ...
            trantor.dbrunner.send('addFollowing', data, dTx, new Date().getTime(), function () {
                setTimeout(function () {

                    updateAuthor(data.followedAddress);
                    trantor.dbrunner.send('getMediaByAuthor', data.followedAddress, data.followerAddress, function (err, result) {
                        if (err) {
                            console.error(err);
                        } else if (result.length > 0) {
    ...
    let onBlock = function () {
        ...
        sendTransaction(txBuffer.toString('hex'), walletPassword, function (err, result) {
            if (err) {
                ...
                trantor.dbrunner.send('getMediaByAddress', data.followedAddress, 'a', function (err, result) {
                    if (err) {
                        console.error(err)
                    } else if (result.length > 0) {
                        onDelete(deleteMedia);
...
function updateAuthor(authorAddress) {
    getUserAddress(function (userAddress) {
        ...
        trantor.dbrunner.send('getMediaAddressByAuthor', authorAddress, function (err, mediaAddresses) {
            if (err) {
                console.error(err);
            } else {
...
function updateMediaItem(address, userAddress, forceAdd = true) {
    trantor.dbrunner.send('getMediaByAddress', address, userAddress, function (err, result) {
...
function downloadAuthorAvatar(authorAddress, avatarMagnet) {
    ...
    trantor.ipfsrunner.send('downloadFile', authorAddress, avatarMagnet, false, function (torrent, file, address) {
        //console.log('author torrent', avatarMagnet);
        let tHash = Utils.makeHash(avatarMagnet);
        trantor.dbrunner.send('putTorrent', tHash, avatarMagnet, torrent.path, file);
        trantor.emit('onAvatarDownloaded', authorAddress, torrent);

        setTimeout(function () {
            getUserAddress(function (userAddress) {
                trantor.dbrunner.send('getMediaByAuthor', authorAddress, userAddress, function (err, result) {
                    if (err) {
                        console.error(err);
                    } else if (result.length > 0) {
...
function downloadPublicFile(contentAddress, magnet) {
    if (magnet && magnet.length > 0) {
        ...
        trantor.ipfsrunner.send('downloadFile', contentAddress, magnet, false, function (torrent, file, contentAddress) {
            let tHash = Utils.makeHash(torrent.CID);
            trantor.dbrunner.send('putTorrent', tHash, magnet, torrent.path, file);
...
function performFollow(address) {
    getUserAddress(function (userAddress) {
        if (address !== userAddress) {
            trantor.dbrunner.send('getFollowingData', userAddress, address, TrantorConstants.TYPE.FOLLOW, function (err, result) {
                if (err) {
                    console.error(err);
                } else if (result.length > 0) {
...
function loadAllMedia() {
    if (!onSearch) {
        if (mustReloadContent) {
            getUserAddress(function (userAddress) {
                trantor.dbrunner.send('getAllMedia', userAddress, discoverPage, function (err, result) {
                    if (err) {
                        console.error(err);
                    } else {
...
function loadMoreDiscoverMedia() {
    discoverPage++;
    getUserAddress(function (userAddress) {
        trantor.dbrunner.send('getAllMedia', userAddress, discoverPage, function (err, results) {
            if (err) {
                console.error(err);
...
function loadMoreFollowingMedia() {
    followingMediaPage++;
    getUserAddress(function (userAddress) {
        trantor.dbrunner.send('getMediaByFollowerAddress', userAddress, userAddress, followingMediaPage, function (err, results) {
...
function loadFollowingMedia() {
    getUserAddress(function (userAddress) {
        trantor.dbrunner.send('getMediaByFollowerAddress', userAddress, userAddress, followingMediaPage, function (err, result) {
            if (err) {
                console.error(err);
            } else {
...
function loadUserFollowedItems() {
    if (mustReloadFollowers) {
        ...
        getUserAddress(function (userAddress) {
            trantor.dbrunner.send('getFollowing', userAddress, userAddress, function (err, result) {
...
function updateUserFollows(followedAddress) {
    getUserAddress(function (userAddress) {
        trantor.dbrunner.send('getFollowingData', userAddress, followedAddress, TrantorConstants.TYPE.FOLLOW, function (err, result) {
...
function prepareArticle(mediaAddress, userAddress) {
    ...
    trantor.dbrunner.send('getMediaByAddress', mediaAddress, userAddress, function (err, result) {
        console.log(result);
        ...
        trantor.dbrunner.send('getComments', mediaAddress, function (err, comments) {
            if (err) {
                console.error(err);
            } else {
...
function downloadPrivateFile(contentAddress, privateContent) {
    ...
    trantor.dbrunner.send('setMediaPrivateContent', contentAddress, privateContent);

    trantor.ipfsrunner.send('downloadFile', contentAddress, privateContent, true, function (torrent, file, contentAddress) {
        let tHash = Utils.makeHash(torrent.CID);

        trantor.dbrunner.send('putTorrent', tHash, torrent.CID, torrent.path, file);
...
function makePayment() {
    ...
    if (mustBeDownloadContent()) {
        ...
        getUserAddress(function (userAddress) {
            trantor.dbrunner.send('getPayment', userAddress, preparedArticle.address, function (err, result) {
                if (err) {
                    console.error(err);
                } else {
                    if (result[0]) {
