...
function loadValues() {
    ...
    let actionAmount = parseFloat(settings.getKey('action-amount', TX_CONTENT_AMOUNT.toPlainString()));
    actionAmount = Math.max(actionAmount, parseFloat(TX_CONTENT_AMOUNT.toPlainString()));
    ...
    $('#settings-action-amount').val(actionAmount)
        .attr('min', TX_CONTENT_AMOUNT.toPlainString());
...
function save() {
    ...
    trantor.events.emit('onNotification', lang.Settings, lang.AppliedChangedSettings, './assets/img/settings.png', 10);
}