...
setInterval(function () {
    info('nexData: ' + nextData + ', size: ' + datas.length);
    if (nextData) {
...
function processTorrentQueue() {
    ...
    let seedTorrent = function (queuedTorrent) {

    };

    if (client.torrents.length === 0) {
        seedTorrent(torrentQueue[0])
    } else if (client.torrents.length < 5) {
        let index = randomNumber(0, torrentQueue.length -1);
