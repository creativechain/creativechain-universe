...
trantor.events.on('onAfterRegister', function (txBuffer, userReg, torrent) {
    ...
    trantor.database.getAuthor(userReg.address, 'a', function (err, result) {
        if (err || result.length === 0) {
            trantor.events.emit('onNotification', lang.RegistrationCompleted, lang.RegistrationCongrats, avatar, 10);
        } else {
            trantor.events.emit('onNotification', lang.ProfileUpdated, lang.YourProfileUpdated, avatar, 10);
        }
