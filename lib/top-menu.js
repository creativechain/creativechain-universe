...
trantor.on('core.register.sent', function (txBuffer, userReg, torrent) {
    ...
    trantor.dbrunner.send('getAuthor', userReg.address, 'a', function (err, result) {
        if (err || result.length === 0) {
            trantor.emit('onNotification', lang.RegistrationCompleted, lang.RegistrationCongrats, avatar, 10);
        } else {
            trantor.emit('onNotification', lang.ProfileUpdated, lang.YourProfileUpdated, avatar, 10);
        }

        trantor.dbrunner.send('addAuthor', userReg, tx, new Date().getTime(), function (result) {
            setTimeout(function () {
                refreshUserData();
...
function performSearch(words) {
    ...
    getUserAddress(function (userAddress) {
        ...
        if (wordsSearch.startsWith('@')) {
            ...
            if (tags.length > 0) {
                //drp.dropdown('toggle');
                trantor.dbrunner.send('getAuthorsByTags', tags, userAddress, function (err, result) {
                    if (err) {
                        console.error(err);
                    } else {
                        if (!userSearchList) {
