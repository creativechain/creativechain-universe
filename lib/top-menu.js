...
trantor.on('core.register.sent', function (txHex, userReg, avatarCID) {
    let tx = DecodedTransaction.fromHex(txHex, trantor.configuration.network);
...
function showDialogPass(title = lang.EnterPassword) {
...
function startSession(password, retries = 0) {
    trantor.rpcWallet.walletPassphrase(password, /*SESSION_DURATION_MILLIS / 1000*/ 10, function (err, result) {
        ...
        if (err) {
            ...
            modal.hide();
            trantor.emit('core.password', password);
        }
    });
}
