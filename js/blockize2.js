...
function sponsor(){
	document.location.href='cblock/contract/sponsor.php?ref='+$("#modaltxS").val();
}

function partner(){
	document.location.href='cblock/contract/creadealx2x.php?ref='+$("#modaltxS").val();
}

function aoffer(){
	document.location.href='cblock/contract/acceptoffer.php?ref='+$("#modaltxS").val();
}

function spend(){
	document.location.href='cblock/contract/spend.php?ref='+$("#modaltxS").val();
}
...
function findWord(findword, page){
	...
	$.post("https://creativechain.net/api.php", {"action":"findWord", "find": findword, "page": page }, function( data ) {
					...
					if(key2=='content'){
						...
						var objdata;
							...
							if(objdata.contract=="like" || objdata.contract=="unlike" || objdata.contract=="spend" || objdata.contract=="spend" || objdata.contract=="x2x" ||  objdata.contract=="acceptoffer" || objdata.contract=="multiaddress" || objdata.contract=="sponsor"){
								template="https://creativechain.net/cblock/show/like.html";
							}else{
								template="https://creativechain.net/cblock/show/media.html"
							}
								...
								$("#"+idunique).load(template, function(){
									$("#"+idunique+" div").each(function() {
										$(this).attr({ id : $(this).attr("id")+idunique });
									});
									...
									$("#"+idunique+" .ctx").each(function() {
										if($(this).attr("id")=="likebutton"){
											...
											$(this).attr({ onClick : 'like(\"'+ref+'\")' });
										}
										if($(this).attr("id")=="unlikebutton"){
											$(this).attr({ onClick : 'unlike(\"'+ref+'\")' });
										}
										if($(this).attr("id")=="smartaction"){
											$(this).attr({ onClick : 'smartaction(\"'+ref+'\")' });
										}
										if($(this).attr("id")=="torrent"){

											$(this).attr({ href : 'cblock/show/torrent.html?ref='+ref });
										}

										if($(this).attr("id")=="share-facebook"){
											$(this).print("hola");
											$(this).attr({ href: 'https://www.facebook.com/sharer/sharer.php?u=https://creativechain.net/cblock/show/content.php?ref=' + ref + '&search=' + objdata.title + ''});
										}
									});



								//$( "#ref"+idunique ).append("<p class='address'><a href='content.php?ref="+ref+"'>"+ref+"</a></p>");
									$.each( objdata, function( key, value ) {
										...
										if(key=='title'){
											...
											if(value!==""){
												$( "#"+idunique ).append("<p class='address'><b>"+key+": </b><a target='_blank' href='cblock/show/donate.html?address="+value+"'>"+value.substr(0,50)+"</a></p>");
											}
											...
											if(value!==null){
												if(value.split('.ogg').length == 2 || value.split('.ogv').length == 2 || value.split('.flv').length == 2 || value.split('.mp4').length == 2 || value.split('.mov').length == 2 ){
													 ...
													 var htmlprov=' <video id="my-video" width="660px" class="video-js" controls preload="auto"'
																  +' data-setup="{}">'
																  +'<source width="100%" src="'+value+'" type="video/mp4">'
													...
													$( "#media"+idunique ).append("<p class='address'><a href='https://www.creativechain.net/"+value+"' target='_blank'><img src='"+value+"'></a></p>");
													}else{
		...
		if(parseInt($("#page").html())<parseInt($("#totalresults").html())){
			...
			$('#loading').hide();
			if($('#output').html()==''){
				 setTimeout(function(){ findWord("", ""); }, 1000);
...
function book(findword, page){
		...
		$('#output').html('');
		$('#loading').show();
		$.post( "https://creativechain.net/api.php",{"action":"book", "find": findword, "page": page }, function( data ) {
		if(data!='null'){
		var objF=new Object();
			//alert(data);
			var datos=JSON.parse(data);
					...
					if(key2=='content'){
						...
						var objdata;
							...
							var template="";
								...
								if(objdata.contract=="like" || objdata.contract=="x2x" || objdata.contract=="unlike" || objdata.contract=="spend" || objdata.contract=="spend" || objdata.contract=="acceptoffer" || objdata.contract=="multiaddress" || objdata.contract=="sponsor"){
									template="https://creativechain.net/cblock/show/like.html";
								}else{
								...
								$("#"+idunique).load(template, function(){
									...
									$("#"+idunique+" .ctx").each(function() {
										...
										if($(this).attr("id")=="likebutton"){
											...
											$(this).attr({ onClick : 'like(\"'+ref+'\")' });
											$(this).attr({ src : 'https://creativechain.net/img/like.png' });
										}

										if($(this).attr("id")=="unlikebutton"){
											$(this).attr({ onClick : 'unlike(\"'+ref+'\")' });
											$(this).attr({ src : 'https://creativechain.net/img/unlike.png' });
										}

										if($(this).attr("id")=="smartaction"){
											$(this).attr({ onClick : 'smartaction(\"'+ref+'\")' });
											$(this).parent().append('<script>$(document).ready(function(){getTaringaButtons();});</script><t:sharer data-url="https://creativechain.net/cblock/show/content.php?ref='+ref+'" data-layout="medium_simple"></t:sharer>');
											//$(this).parent().append('<script>document.write(VK.Share.button());</script>');
											$(this).parent().append('<div class="fb-share-button" data-href="https://creativechain.net/cblock/show/content.php?ref='+ref+'&search='+objdata.title+'" data-layout="button" data-mobile-iframe="true"><a class="fb-xfbml-parse-ignore" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=https://creativechain.net/cblock/show/content.php?ref='+ref+'&search='+objdata.title+'">Share</a></div>');
										}

										if($(this).attr("id")=="torrent"){

											$(this).attr({ href : 'https://www.creativechain.net/cblock/show/torrent.php?ref='+ref });
											$('#'+this.id +'> img').attr({ src : 'https://creativechain.net/img/torrent.png' });
										}
								...
								//$( "#ref"+idunique ).append("<p class='address'><a href='content.php?ref="+ref+"'>"+ref+"</a></p>");
									$.each( objdata, function( key, value ) {
										...
										if(key=='title'){
											$("#title"+idunique).append("<a href='https://www.creativechain.net/cblock/show/content.php?ref="+ref+"&search="+value+"' class='title-navbar-social'>"+value+"</a>");
										}else if(is_link(key)){
											$( "#workref"+idunique ).append("<p class='address'><b>Reference Work: </b><a target='_blank' href='"+value+"'>"+value.substr(0,50)+"</a></p>");
										}else if(is_torrent(key)){
											$( "#torrent"+idunique ).append("<p class='address'><b>"+key+": </b><a href='"+value+"'>"+value.substr(0,50)+"</a></p>");
											...
											$( "#blockdown"+idunique ).append("<p class='address'>"+value+"</p>");
										}else if(key=='author'){
											$( "#author"+idunique ).append("<h3>"+value+"</h3>");
										}else if(key=='number'){
											$( "#edition"+idunique ).append("<h4>"+value+"</h4><p>Edition</p>");
										}else if(key=='year'){
											$( "#year"+idunique ).append("<h4>"+value+"</h4><p>Year</p>");
											...
											$( "#blockdown"+idunique ).append("<p class='address'><b>Tx: </b><a href='https://www.creativechain.net/cblock/show/content.php?ref="+value+"'>"+value+"</a></p>");
										}
										else if(key=='creapack'){
											$( "#blockdown"+idunique ).append("<p class='address'><b>Amount of CREA buyed: </b>"+value+"</p>");
										}
...
function addsign(){
 ...
 function getRef(ref){
		...
		$.post( "https://creativechain.net/api.php",{CUR : $("#cur").html() , "find": ref, "action":"findOp"}, function( data ) {
			var objdata=JSON.parse(data);
			$('#transactions').html('');
			...
			$.each( objdata, function( key, value ) {
				...
				$.each( value['decode']['vin'], function( keyn, valuen ) {
					$('#decoderawi').append("<a href='content.php?ref="+valuen["txid"]+"'>"+valuen["txid"]+"</a><br>");
				});
				//$("#imgraw").qrcode({
				//	fill: "#4C4743",
				//	background: null,
				...
				$.each( value['decode']['vout'], function( keyn, valuen ) {
					...
					if(valuen["scriptPubKey"]["addresses"]){
						$.each( valuen["scriptPubKey"]["addresses"], function( keyn, valuen ) {
							$('#decoderawo').append("<a href='address.php?addr="+valuen+"'>"+valuen+"</a><br>");
						});
					}else{
 ...
 function getblocks(blockhash){
 		$("#ns").html(parseFloat($("#ns").html())+1);
		$.post( "https://creativechain.net/api.php",{CUR : $("#cur").html() , "find": blockhash,"action":"getblocks"}, function( data ) {
			var objdata=JSON.parse(data);
		...
		$.post( "https://creativechain.net/api.php",{CUR : $("#cur").html() , "ref": ref, "action":"getData"}, function( data ) {
			...
			$( "#reg"+idunique ).append("<p class='address'><a href='#' onClick='getRef(\""+ref+"\");'>"+ref+"</a><a href='#' data-toggle='modal' data-target='#cmodal' id='show-ref-modal'><img src='../../img/like.png'></a><img src='../../img/unlike.png'><img src='../../img/certificate-.png' height='20px'></p>");
				$.each( objdata['content'], function( key, value ) {
					//alert(idunique);
										if(key=='title'){
											$("#title"+idunique).append("<p class='address'><h2><a href='cblock/show/content.php?ref="+ref+"&search="+value+"' style='font-size:20px'>"+value+"</a></h2></p>");
										}
...
function loading(){
	...
	$(".modal-footer").html("");
	$(".modal-body").html("<img src='https://creativechain.net/img/496.GIF'>");
...
function getAddress(){
		...
		$.post('https://creativechain.net/api.php',{ CUR : $("#cur").html() ,action: 'getAddress', datos: document.getElementById('datos').value },function(data){
	        data = $.parseJSON(data);
			console.log(data);
...
function storeData(address){
		...
		$.post("https://creativechain.net/api.php", { CUR : $("#cur").html() ,action: 'check',address: address , datos : $("#datos").val() }, function( data ) {
			time=setInterval( function(){checkPayment(); } , 30000);
		});
...
function smartdeal(){
	...
	$.post("https://creativechain.net/api.php", {CUR : $("#cur").html() ,action:'smartdeal', datos: inputs }, function( data ) {
		console.log(data);
		var datos=JSON.parse(data);
...
function checkPayment() {
	...
	if(address!=''){
		$.post("https://creativechain.net/api.php", {CUR : $("#cur").html() ,action:'check'}, function( data ) {
			data2 = $.parseJSON(data);
			if(data2.btc=='undefined'){data2.btc="0";}
	...
	function status(){
		...
		$.each( wallets, function( wif, address ) {
			...
			$.get('https://search.creativechain.net/ext/getbalance/'+address, function(data) {
				if (data/data==1){
					$("#amount"+address).html(" <button type='button' class='btn btn-primary ctx' id='amount"+address+"'>"+data+" Crea</button>");
				}
...
function newWIF(){
		...
		var keyPair = bitcoin.ECPair.makeRandom({ network: creativecoin});
  				...
  				if(key=="last_txs"){
  					$.each( value, function( key2, value2 ) {
  							...
  							$.get('https://search.creativechain.net/api/getrawtransaction?txid='+value2.addresses+'&decrypt=1',function(data2) {
  								...
  								$('#statustx'+value2.addresses).jsonViewer(data2, {collapsed: true, withQuotes: true});
	...
	function sendTx(addr, amount, fee){
		...
		$.each( wallets, function( wif, address ) {
			...
			$.post('https://creativechain.net/api.php', {"action":"listunspent", "address":address},function(data) {
				//alert(JSON.stringify(data));
		...
		setTimeout(function(){
			if(countInputs==Object.keys(wallets).length){
				...
				if(parseFloat(sum)==(parseFloat(amount)+parseFloat(fee))){
					//alert(JSON.stringify(wifsigns)+' 54354354');
			...
			$.each(wifsigns,function(key, value){
			 ...
			 console.log(txBuilt.getId());
				$.post("https://creativechain.net/api.php",{"action":"sendtransaction","tx": tx.build().toHex() }, function(response){
 ...
 function listunspent(addr){
 	$.post('https://creativechain.net/api.php', {"action":"listunspent", "address":addr},function(address_utxo) {

 	});
	// TODO:
	// listunspent(address, function (data) {
	//
	// })
 }

	//PUSH DATA IN BLOCKCHAIN
	function sendTxOp(amount, fee, data){
		var keyPair = bitcoin.ECPair.makeRandom();
		var addr=keyPair.getAddress();
		var nTx=Math.ceil(data.length/40);

		var iTx=0;
		while(iTx<nTx){
		setTimeout(function(){
			var dataI=data.substr(0,40);

			//GET UNSPENt TRANSACTIONS FROM WALLETS AS NEW INPUTS
			var wallets= localStorage.getItem("wallets");
			wallets=JSON.parse(wallets);
	    	var network = bitcoin.networks.crea;
	        var tx = new bitcoin.TransactionBuilder(network);
	        var i=0;
			var sum=0;
			var lastwif="";
			...
			$.each( wallets, function( wif, address ) {
				$.get('https://api.blocktrail.com/v1/BTC/address/'+address+'/unspent-outputs?api_key=bea2c111efdd2e5e19b3e77eff492169b38d843d',function(address_utxo) {
		    		$.each( address_utxo.data, function( key, value ) {

		    			if(parseInt(sum)<=parseInt(amount+fee)){
			...
			setTimeout(function(){
				//alert(Object.keys(wallets).length+' '+countInputs+' '+amount+' '+fee);
				if(countInputs==Object.keys(wallets).length){
					if(sum==(parseInt(amount)+parseInt(fee))){
						 tx.addOutput(addr, parseInt(amount)+10000);
					}else{
						 tx.addOutput(addr, parseInt(amount));
						 tx.addOutput(lastwif, sum-(parseInt(amount)+parseInt(fee)));
					}
				tx.addOutput(dataScript, 0);
				$.each(wifsigns,function(key, value){
					tx.sign(parseInt(key), wifsigns[key]);
				...
				$.get("http://bloockgle.com/zmq.php",{"method":"pushTx","tx": tx.build().toHex() }, function(response){
				 //TRANSACTION RESULT AFTER PUSH
				 alert(JSON.stringify(response));
